            ...
            db.all(
              ...
              (err3, rows2) => {
                if (err3) {
                  ...
                  console.log(
                    `   Corretti (/uploads/...): ${rows2[0].corretti}`
                  );
                  console.log(
                    `   Ancora sbagliati: ${rows2[0].ancora_sbagliati}\n`
                  );
                }

                // 5. Mostra alcuni esempi
                db.all(
                  `SELECT id, entita_riferimento, url FROM IMMAGINI WHERE url LIKE '/uploads/%' LIMIT 5`,
                  (err4, examples) => {
                    if (!err4 && examples.length > 0) {
                      console.log("📸 Esempi di URL corretti:");
                      examples.forEach((ex) => {
                        console.log(
                          `   ${ex.entita_riferimento} (ID ${ex.id}): ${ex.url}`
                        );
                      });
                    }
                    db.close();
