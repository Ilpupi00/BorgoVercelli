...
async function applyMigration() {
  try {
    ...
    const migrationPath = path.join(
      __dirname,
      "../database/migrations/add_annullata_da_column.sql"
    );
    ...
    console.log(
      "   - Aggiunta colonna annullata_da (VARCHAR(10)) alla tabella PRENOTAZIONI"
    );
    console.log(
      '   - La colonna traccia se l\'annullamento è stato fatto da "user" o "admin"'
    );
    console.log(
      "   - Gli utenti possono riattivare solo le prenotazioni annullate da loro stessi"
    );
    console.log("");
