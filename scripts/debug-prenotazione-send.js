...
(async () => {
  try {
    ...
    try {
      const r = await db.query("SELECT nome FROM campi WHERE id = $1", [
        campo_id,
      ]);
      if (r && r.rows && r.rows[0] && r.rows[0].nome)
        campoNome = r.rows[0].nome;
    } catch (e) {
      // Non bloccante
    }
    ...
    if (utente_id) {
      ...
      const resUser = await pushService.sendNotificationToUsers(
        [Number(utente_id)],
        {
