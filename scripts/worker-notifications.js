...
async function processNotification(notification) {
  ...
  try {
    ...
    console.log(
      `[WORKER] Dettagli: tipo=${type}, attempt=${attempts + 1}/${
    ...
    try {
      ...
      console.log(
        `[WORKER] Payload validato: "${
          payloadObj.title
        }" - ${payloadObj.body.substring(0, 50)}...`
      );
    } catch (parseError) {
    ...
    const sendPromise = (async () => {
      switch (type) {
        ...
        case "user":
          ...
          console.log(
            `[WORKER] Invio a ${user_ids.length} utent${
              user_ids.length === 1 ? "e" : "i"
            }: [${user_ids.join(", ")}]`
          );
    ...
    processedCount++;
    console.log(
      `[WORKER] ✅ Notifica ${id} inviata con successo (sent: ${
        result?.sent || 0
    ...
    if (error.message.includes("VAPID") || error.message.includes("403")) {
      errorType = "vapid";
      console.error(
        `[WORKER] ⚠️ Errore VAPID - verificare configurazione chiavi`
      );
    } else if (
      ...
      console.error(
        `[WORKER] 🚫 Payload non valido - notifica verrà marcata come failed`
      );
    }
    ...
    if (!shouldRetry || newAttempts >= maxAttempts) {
      ...
      console.log(
        `[WORKER]    Motivo: ${
          !shouldRetry
            ...
            : `${newAttempts}/${maxAttempts} tentativi esauriti`
        }`
      );
    } else {
      ...
      console.log(
        `[WORKER] 🔄 Notifica ${id} -> PENDING (retry ${newAttempts}/${maxAttempts} tra ${nextRetryIn}s)`
      );
    }
...
async function processLoop() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `[WORKER] Subscriptions cache caricata: ${subscriptionsCache.length}`
      );
    } catch (e) {
      console.error(
        "[WORKER] Errore caricamento subscriptions cache:",
        e && e.message
      );
    ...
    console.log(
      `[WORKER] Trovate ${notifications.length} notifiche da processare`
    );
    ...
    const runNext = async () => {
      ...
      try {
        ...
        console.error(
          "[WORKER] Errore durante processNotification:",
          e && e.message
        );
      }
      return runNext();
...
async function cleanupOldNotifications() {
  try {
    ...
    if (resetCount > 0) {
      console.log(
        `[WORKER] 🔓 Reset ${resetCount} notifiche stuck in 'sending'`
      );
    ...
    if (deleted > 0) {
      console.log(
        `[WORKER] 🗑️  Rimosse ${deleted} notifiche vecchie (>${CONFIG.CLEANUP_AFTER_DAYS} giorni)`
      );
    }
    ...
    if (resetCount === 0 && deleted === 0) {
      console.log(
        "[WORKER] ✨ Cleanup completato - nessuna notifica da pulire"
      );
    }
  } catch (error) {
...
async function printStats() {
  try {
    ...
    result.rows.forEach((row) => {
      console.log(
        `  ${row.status}: ${row.count} (oldest: ${
          row.oldest?.toISOString() || "N/A"
        })`
      );
    });
    console.log("");
...
async function main() {
  ...
  try {
    ...
    console.error(
      "[WORKER] ❌ Tabella notifications non trovata. Esegui la migration prima!"
    );
    process.exit(1);
  }
