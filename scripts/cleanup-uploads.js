...
console.log(`⚙️  Modalità: ${isDryRun ? 'DRY RUN (simulazione)' : 'ELIMINAZIONE REALE'}`);
console.log('='.repeat(50));
console.log('');
...
async function cleanup() {
  try {
    ...
    if (orphanFiles.length === 0) {
      ...
      console.log('✨ Volume uploads pulito e sincronizzato con il database.\n');
      return;
    }
    ...
    orphanFiles.forEach((file, index) => {
      ...
      console.log(`      📅 Ultima modifica: ${stat.mtime.toLocaleString('it-IT')}`);
