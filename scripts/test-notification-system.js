...
async function testNotificationSystem() {
  ...
  try {
    ...
    const tableCheck = await db.query(`
        ...
        `);
    console.log(
      `✅ Tabella notifications trovata con ${tableCheck.rows.length} colonne:`
    );
    ...
    if (subsStats.total === 0) {
      ...
      console.log(
        "   Gli utenti devono abilitare le notifiche dal browser prima di riceverle.\n"
      );
    }
    ...
    if (TEST_QUEUE) {
      ...
      const result = await notifications.queueNotificationForAdmins(
        testPayload,
        { priority: 1 }
      );
      ...
      console.log(
        "5️⃣ Test accodamento saltato (imposta TEST_QUEUE=true per abilitare)\n"
      );
    }
    ...
    if (recentResult.rows.length === 0) {
      ...
      recentResult.rows.forEach((notif, idx) => {
        ...
        if (notif.processing_time_seconds) {
          console.log(
            `      Tempo elaborazione: ${notif.processing_time_seconds.toFixed(
              2
            )}s`
          );
        }
        if (notif.last_error) {
          console.log(
            `      Ultimo errore: ${notif.last_error.substring(0, 80)}...`
          );
        }
    ...
    if (stuckResult.rows.length > 0) {
      console.log(
        `⚠️  Trovate ${stuckResult.rows.length} notifiche stuck in 'sending':`
      );
    ...
    console.log(
      `✅ Tabella notifications: OK (${tableCheck.rows.length} colonne)`
    );
    console.log(
      `📊 Notifiche in coda: ${stats.pending} pending, ${stats.sending} sending`
    );
    console.log(
      `📱 Subscriptions attive: ${subsStats.total} (${subsStats.admin_count} admin, ${subsStats.user_count} utenti)`
    );

    if (subsStats.total === 0) {
      console.log(
        `⚠️  NOTA: Nessuna subscription - le notifiche non verranno recapitate`
      );
    }
