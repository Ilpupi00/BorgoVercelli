...
async function fixPaths() {
  ...
  try {
    ...
    console.log(
      `✅ Aggiornati ${result1.rowCount} record (src/public/uploads/ → /uploads/)\n`
    );
    ...
    if (result2.rowCount > 0) {
      console.log(
        `✅ Corretti ${result2.rowCount} percorsi senza slash (uploads/ → /uploads/)\n`
      );
    }
    ...
    if (correctedExamples.rows.length > 0) {
      ...
      correctedExamples.rows.forEach((ex) => {
        console.log(
          `   ${ex.tipo || ex.entita_riferimento} (ID ${ex.id}): ${ex.url}`
        );
      });
    }
    ...
    console.log(
      "   Ora puoi fare il deploy su Railway con i percorsi corretti.\n"
    );
  } catch (error) {
    console.error("❌ Errore durante la correzione:", error.message);
    if (error.code === "ENOTFOUND") {
      console.error(
        "   Verifica che DATABASE_URL sia corretta e che il database sia raggiungibile."
      );
    }
  } finally {
    client.release();
