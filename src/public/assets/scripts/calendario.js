...
(function () {
  ...
  function toggleTheme() {
    const currentTheme =
      localStorage.getItem("site-theme-preference") || "auto";
    let newTheme;
  ...
  function showDayModal(date, bookings) {
    if (!elements.dayModal || !elements.modalTitle || !elements.modalBody)
      return;
    ...
    if (bookings.length === 0) {
      ...
      elements.modalBody.innerHTML = bookings
        .map((booking) => {
          const statoLabel = {
          ...
          return `
                    <div class="booking-card ${booking.stato}">
                        <div class="booking-header">
                            <div class="booking-campo">${
                              booking.campo_nome || "Campo " + booking.campo_id
                            }</div>
                            <span class="booking-status ${booking.stato}">${
            statoLabel[booking.stato] || booking.stato
          }</span>
                        </div>
                        <div class="booking-details">
                            <div class="booking-detail">
                                <i class="bi bi-clock"></i>
                                <span>${booking.ora_inizio} - ${
            booking.ora_fine
          }</span>
                            </div>
                            ${
                              booking.utente_nome
                                ...
                                <div class="booking-detail">
                                    ...
                                    <span>${booking.utente_nome} ${
                                    booking.utente_cognome || ""
                                  }</span>
                                </div>
                            `
                                : ""
  ...
  async function handleExportSubmit(e) {
    ...
    try {
      ...
      submitBtn.innerHTML =
        '<i class="bi bi-hourglass-split me-2"></i>Generazione in corso...';

      // Download file
      const response = await fetch(
        `/prenotazione/export-report?${params.toString()}`
      );
