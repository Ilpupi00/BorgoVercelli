class CampoManager {
  ...
  async handleDelete(e, btn) {
    ...
    if (window.ShowModal && typeof ShowModal.modalDelete === "function") {
      const confirmed = await ShowModal.modalDelete(
        "Sei sicuro di voler eliminare questo campo? Questa operazione è irreversibile.",
  ...
  populateEditForm(campo) {
    ...
    document.getElementById("edit_tipo_superficie").value =
      campo.tipo_superficie || "";
    document.getElementById("edit_dimensioni").value = campo.dimensioni || "";
    document.getElementById("edit_capienza_pubblico").value =
      campo.capienza_pubblico || "";
    document.getElementById("edit_descrizione").value = campo.descrizione || "";
    document.getElementById("edit_illuminazione").checked =
      !!campo.illuminazione;
  ...
  setupSaveHandler() {
    document
      .getElementById("saveEditCampoBtn")
  ...
  async handleSave() {
    ...
    formData.append(
      "indirizzo",
      document.getElementById("edit_indirizzo").value
    );
    formData.append(
      "tipo_superficie",
      document.getElementById("edit_tipo_superficie").value
    ...
    formData.append(
      "dimensioni",
      document.getElementById("edit_dimensioni").value
    );
    formData.append(
      "capienza_pubblico",
      document.getElementById("edit_capienza_pubblico").value
    ...
    formData.append(
      "illuminazione",
      document.getElementById("edit_illuminazione").checked ? 1 : 0
    );
    formData.append(
      "coperto",
      document.getElementById("edit_coperto").checked ? 1 : 0
    );
    formData.append(
      "spogliatoi",
      document.getElementById("edit_spogliatoi").checked ? 1 : 0
    );
    formData.append(
      "docce",
      document.getElementById("edit_docce").checked ? 1 : 0
    );
    formData.append(
      "attivo",
      document.getElementById("edit_attivo").checked ? 1 : 0
    );
    ...
    try {
      ...
      if (result && result.success) {
        ...
        const alert = document.createElement("div");
        alert.className = "alert alert-success mt-3";
        alert.textContent = result.updated
  ...
  updateTableRow(id) {
    ...
    if (row) {
      ...
      badgeCell.innerHTML = document.getElementById("edit_attivo").checked
