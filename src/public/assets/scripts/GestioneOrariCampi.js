...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("click", function (e) {
    const addBtn = e.target.closest(
      '[data-action="add-default"], [data-action="add-day"]'
    );
    if (!addBtn) return;
    ...
    document.getElementById("modalGiornoSettimana").value =
      currentGiornoSettimana || "";
    document.getElementById("ora_inizio").value = "";
    document.getElementById("ora_fine").value = "";

    const modal = new bootstrap.Modal(
      document.getElementById("addOrarioModal")
    );
    modal.show();
  });
  ...
  const cancelBtn = document.querySelector(
    '#addOrarioModal [data-bs-dismiss="modal"]'
  );
  if (cancelBtn) {
...
async function addOrario() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      window.AdminGlobal.ToastManager.show(
        msg || "Errore nell'aggiunta dell'orario",
        "error"
      );
    }
  } catch (error) {
...
async function updateOrario(id, field, value) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      window.AdminGlobal.ToastManager.show(
        result.error || "Errore nell'aggiornamento",
        "error"
      );
...
async function deleteOrario(id) {
  ...
  try {
    ...
    if (response.ok) {
      window.AdminGlobal.ToastManager.show(
        "Orario eliminato con successo",
        "success"
      );
      setTimeout(() => location.reload(), 1500);
    } else {
      window.AdminGlobal.ToastManager.show(
        data.error || "Errore nella cancellazione",
        "error"
      );
    }
  } catch (error) {
