...
class NewsEditor {
  ...
  loadExistingContent() {
    if (window.notiziaData && window.notiziaData.contenuto) {
      console.log(
        "📝 Caricamento contenuto esistente:",
        window.notiziaData.contenuto.substring(0, 100) + "..."
      );
  ...
  checkForChanges() {
    ...
    if (
      this.lastContent &&
      JSON.stringify(currentContent) !== JSON.stringify(this.lastContent)
    ) {
    ...
    if (form) {
      ...
      form.addEventListener("submit", (e) => {
        ...
        if (contenutoElement && this.quill) {
          ...
          console.log(
            "✅ Contenuto aggiornato prima del submit:",
            contenutoHtml.substring(0, 100) + "..."
          );
        }
  ...
  validateForm() {
    ...
    if (!contenutoElement.value || contenutoElement.value.trim() === "") {
      ...
      alert(
        "Il contenuto della notizia è obbligatorio. Scrivi qualcosa nel testo della notizia."
      );
      this.quill.focus();
      return false;
    }
  ...
  if (saveBtn && cropperInstance) {
    saveBtn.onclick = async () => {
      ...
      canvas.toBlob(async (blob) => {
        ...
        alert(
          "Modifiche salvate! L'immagine verrà caricata al salvataggio della notizia."
        );
      });
    };
  }
