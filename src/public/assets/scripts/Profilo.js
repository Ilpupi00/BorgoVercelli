class Profilo {
  ...
  setupEventListeners() {
    ...
    const editProfileTriggers = document.querySelectorAll(
      '[data-action="edit-profile"], [onclick="modificaPreferenze()"]'
    );
    ...
    const changePasswordTriggers = document.querySelectorAll(
      '[data-action="change-password"], [onclick="cambiaPassword()"]'
    );
  ...
  modificaPreferenze() {
    ...
    if (this.currentUser) {
      ...
      if (this.currentUser.codice_fiscale) {
        document.getElementById("editCodiceFiscale").value =
          this.currentUser.codice_fiscale.toUpperCase();
      }

      // Preferenze sportive
      document.getElementById("ruoloPreferito").value =
        this.currentUser.ruolo_preferito || "";
      document.getElementById("piedePreferito").value =
        this.currentUser.piede_preferito || "";
    }
  ...
  mostraRecensioniUtente(recensioni) {
    ...
    recensioni.forEach((recensione) => {
      const stelle =
        "★".repeat(recensione.valutazione) +
        "☆".repeat(5 - recensione.valutazione);
      const data = new Date(recensione.data_recensione).toLocaleDateString(
        "it-IT"
      );
  ...
  async caricaNotizieEventiUtente() {
    ...
    if (!container) {
      console.warn(
        "Profilo.caricaNotizieEventiUtente: #userNewsEventsContainer non trovato, skip."
      );
      if (countBadge) {
  ...
  mostraNotizieEventiUtente(notizie, eventi) {
    ...
    if (!container) {
      console.warn(
        "Profilo.mostraNotizieEventiUtente: #userNewsEventsContainer non trovato, skip rendering."
      );
      if (countBadge) {
        const totalItems =
    ...
    eventi.forEach((evento) => {
      const data = new Date(
        evento.data_inizio || evento.data_evento
                    ...
                    <div class="d-flex align-items-center flex-grow-1">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <p class="mb-1 text-muted small text-truncate">${
                              evento.descrizione
                                ? evento.descrizione.substring(0, 80) +
                                  (evento.descrizione.length > 80 ? "..." : "")
                                : "Nessuna descrizione"
                            }</p>
                            <small class="text-muted">
                                ...
                                ${
                                  evento.luogo
                                    ? `<i class="bi bi-geo-alt ms-2 me-1"></i>${evento.luogo}`
                                    : ""
                                }
                                ${
                                  pubblicato
                                    ? '<span class="badge bg-success ms-2">Pubblicato</span>'
                                    : '<span class="badge bg-warning ms-2">Bozza</span>'
                                }
                            </small>
                        </div>
                    </div>
                    <div class="btn-group" role="group">
                        ${
                          evento.id || evento.E_id
                            ? `
                            <a href="/evento/crea-evento/${
                              evento.id || evento.E_id
                            }" class="btn btn-sm btn-outline-success" title="Modifica evento">
                                <i class="bi bi-pencil"></i>
                            </a>
                            <button class="btn btn-sm btn-outline-info" title="Visualizza" onclick="window.open('/evento/${
                              evento.id || evento.E_id
                            }', '_blank')">
                                <i class="bi bi-eye"></i>
                            </button>
                ...
                <div class="news-event-item d-flex align-items-center justify-content-between p-3 border-bottom hover-shadow">
                    <div class="d-flex align-items-center flex-grow-1">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <p class="mb-1 text-muted small text-truncate">${
                              notizia.sottotitolo ||
                              notizia.contenuto.substring(0, 80) +
                                (notizia.contenuto.length > 80 ? "..." : "")
                            }</p>
                            <small class="text-muted">
                    ...
                    <div class="btn-group" role="group">
                        ${
                          notizia.id || notizia.N_id
                            ...
                            <a href="/notizie/edit/${
                              notizia.id || notizia.N_id
                            }" class="btn btn-sm btn-outline-primary" title="Modifica notizia">
                                <i class="bi bi-pencil"></i>
                            </a>
                            <button class="btn btn-sm btn-outline-info" title="Visualizza" onclick="window.open('/notizia/${
                              notizia.id || notizia.N_id
                            }', '_blank')">
                                <i class="bi bi-eye"></i>
                            </button>
  ...
  async handleDeleteReview() {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        alert(
          "Errore nell'eliminazione: " + (result.error || "Errore sconosciuto")
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        document.getElementById(
          "uploadPicMsg"
        ).innerHTML = `<div class="alert alert-danger">Errore: ${errorMsg}</div>`;
      }
    } catch (error) {
