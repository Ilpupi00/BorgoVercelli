class FilterSquadre {
  ...
  async addRoster(squadraId) {
    ...
    const giocatori = this.giocatori.filter(
      (g) => Number(g.squadra_id) === Number(squadraId)
    );
    ...
    giocatori.forEach((giocatore, idx) => {
      ...
      if (giocatore.id_immagine && giocatore.id_immagine.trim() !== "") {
        fotoCell = `<img src="${giocatore.id_immagine}" class="player-photo rounded-circle" style="width:32px;height:32px;object-fit:cover;" alt="Foto Giocatore" onerror="this.onerror=null; this.outerHTML='<i class=\\'fa fa-user-circle fa-2x text-secondary\\'></i>';">`;
      } else {
        fotoCell = `<i class="fa fa-user-circle fa-2x text-secondary"></i>`;
      }
      const ruoloClass = giocatore.ruolo
        ? giocatore.ruolo
            .toLowerCase()
            .replace(/\s+/g, "")
            .replace(/[àèéìòù]/g, "")
      ...
      tr.innerHTML = `
                ...
                <td><span class="position-badge position-${ruoloClass}">${
        giocatore.ruolo || ""
      }</span></td>
                <td>${eta}</td>
                <td>${
                  giocatore.piede_preferito === "Sinistro" ? "SX" : "DX" || ""
                }</td>
