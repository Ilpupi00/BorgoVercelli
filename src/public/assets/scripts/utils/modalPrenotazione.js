 ...
 */
export function showModalPrenotazione(
  campo,
  orariDisponibili,
  dataSelezionata,
  onSubmit
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
          <div class="modal-header bg-primary text-white">
            <h5 class="modal-title"><i class="bi bi-calendar-check me-2"></i>Prenota il campo: ${
            ...
            <div class="booking-section mb-4">
              ...
              <div class="row g-3">
                <div class="col-md-6">
                  ...
                  <input type="date" class="form-control form-control-lg" id="dataPrenotazione" name="dataPrenotazione" required value="${
                    dataSelezionata || new Date().toISOString().slice(0, 10)
                  }">
  ...
  (async () => {
    try {
      ...
      if (resUser.ok) {
        ...
        const codiceFiscaleInput = modal.querySelector(
          "#codiceFiscalePrenotazione"
        );
        const tipoDocumentoSelect = modal.querySelector(
          "#tipoDocumentoPrenotazione"
        );
  ...
  form.addEventListener("submit", async function (e) {
    ...
    if (isCustomMode) {
      ...
      if (ora_inizio >= ora_fine) {
        ...
        oraFineCustom.nextElementSibling.textContent =
      ...
      if (bookingDateTime < minTime) {
        ...
        oraInizioCustom.nextElementSibling.textContent =
      ...
      const existingBadges = Array.from(
        document.querySelectorAll(`#orariDisponibili-${campo.id} .badge`)
      );
      ...
      if (isDuplicate) {
        oraInizioCustom.classList.add("is-invalid");
        oraInizioCustom.nextElementSibling.textContent =
          "Orario già presente tra quelli disponibili";
        return;
      }

      // Chiamata al server per verifica definitiva
      try {
        console.log("[MODAL] Controllo disponibilità orario custom:", {
          campo_id: campo.id,
          data,
          ora_inizio,
          ora_fine,
        ...
        if (!checkResult.ok) {
          ...
          oraInizioCustom.classList.add("is-invalid");
          oraInizioCustom.nextElementSibling.textContent =
            checkResult.message || "Orario non disponibile";
          return;
        }
    ...
    console.log("Dati prenotazione pronti:", {
      campo_id: campo.id,
      data,
      ora_inizio,
      ora_fine,
    ...
    if (!telefono || !phoneRegex.test(telefono)) {
      ...
      if (feedback && feedback.classList.contains("invalid-feedback")) {
        feedback.textContent =
          "Formato non valido. Usa +39 seguito da 9-10 cifre (es: +39 3331234567)";
      }
      return;
    }
  ...
  async function aggiornaOrariDisponibili(data) {
    ...
    try {
      const res = await fetch(
        `/prenotazione/campi/${
          campo.id
        }/disponibilita?data=${data}&_=${Date.now()}`
      ); // aggiungo cache buster
    ...
    if (
      (!Array.isArray(orari) || orari.length === 0) &&
      ...
      try {
        ...
        const res2 = await fetch(
          `/prenotazione/campi/${
            campo.id
          }/disponibilita?data=${tStr}&_=${Date.now()}`
        );
        ...
        if (Array.isArray(orariDomani) && orariDomani.length > 0) {
          ...
          select.innerHTML = orariDomani
            .map(
              (o) =>
                `<option value='${o.inizio}|${o.fine}'>${o.inizio} - ${o.fine} (${campo.nome} - ${tStr})</option>`
            )
            .join("");
          ...
          if (!noteEl) {
            ...
            modal
              .querySelector("#orarioPrenotazione")
              .parentNode.appendChild(noteEl);
          }
          noteEl.textContent =
            "Nessun orario disponibile per la data scelta. Mostrati gli orari disponibili per " +
            tStr +
            ".";
    ...
    select.innerHTML =
      orari.length > 0
        ? orari
            .map(
              (o) =>
                `<option value='${o.inizio}|${o.fine}'>${o.inizio} - ${o.fine} (${campo.nome})</option>`
            )
            .join("")
        : '<option value="">Nessun orario disponibile</option>';
    // Log per controllo opzioni select
    console.log(
      "Select aggiornata, opzioni:",
