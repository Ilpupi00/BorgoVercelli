  ...
  try {
    ...
    if (result && result.success) {
      ...
      if (selectedFile && result.eventoId && method === "POST") {
        console.log(
          "📤 Caricamento immagine per nuovo evento ID:",
          result.eventoId
        );
...
function validateForm() {
  ...
  if (
    maxPartecipanti &&
    (isNaN(maxPartecipanti) || parseInt(maxPartecipanti) < 0)
  ) {
    errors.push(
...
function showErrors(errors) {
  ...
  errorDiv.innerHTML =
    '<i class="bi bi-exclamation-triangle me-2"></i>' + errors.join("<br>");
...
function initializeQuill() {
  ...
  if (editorContainer2 && typeof Quill !== "undefined") {
    ...
    const initialContent2 = editorContainer2.getAttribute(
