...
class NotizieManager {
  ...
  async loadInitialNews() {
    ...
    try {
      ...
      console.log(
        "Data received:",
        data.notizie ? data.notizie.length : "no notizie property",
        "items"
      );
    ...
    if (this.allNews && this.allNews.length > 0) {
      ...
      this.filteredNews = this.allNews.filter((notizia) => {
        ...
        if (dateRange && notizia.data_pubblicazione) {
          ...
          matchesDate =
            newsDate >= dateRange.start && newsDate <= dateRange.end;
        }
  ...
  async populateAuthorFilter() {
    try {
      ...
      const authors = [
        ...new Set(
          this.allNews.map((n) => n.autore).filter((a) => a && a.trim())
        ),
      ];
      const authorSelect = document.getElementById("authorFilter");
  ...
  renderNews() {
    console.log(
      "Rendering news, filteredNews count:",
      this.filteredNews.length
    );
    const container = document.getElementById("newsGrid");
  ...
  createNewsCard(notizia) {
    ...
    const imageUrl =
      ...
      notizia.immagine.url.trim() !== ""
        ? notizia.immagine.url
        : "/assets/images/Campo.png";
    const dateStr = new Date(notizia.data_pubblicazione).toLocaleDateString(
      "it-IT",
      {
        year: "numeric",
        month: "long",
    ...
    const excerpt = notizia.contenuto
      ? notizia.contenuto.replace(/<[^>]*>/g, "").substring(0, 120) + "..."
      : "Nessuna descrizione";
