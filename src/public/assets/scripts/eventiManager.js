...
class EventiManager {
  ...
  async checkForMoreEvents() {
    try {
      ...
      const publishedEvents = data.eventi.filter(
        (event) => event.pubblicato === 1 || event.pubblicato === true
      );
  ...
  async loadEvents() {
    ...
    try {
      ...
      this.allEvents = allEvents.filter(
        (event) => event.pubblicato === 1 || event.pubblicato === true
      );
  ...
  async loadMoreEvents() {
    ...
    try {
      ...
      const currentEventCards =
        this.eventsGrid.querySelectorAll(".event-card").length;
      ...
      // Filtra solo gli eventi pubblicati
      const publishedEvents = this.allEvents.filter(
        (event) => event.pubblicato === 1 || event.pubblicato === true
      );
      ...
      nextBatch.forEach((event, index) => {
        const eventCard = this.createEventCard(
          event,
          template,
  ...
  applyFilters(searchTerm = "") {
    this.filteredEvents = this.allEvents.filter((event) => {
      ...
      const matchesSearch =
        ...
        (event.descrizione &&
          event.descrizione.toLowerCase().includes(searchTerm)) ||
  ...
  renderEvents(eventsToRender = null, clearGrid = true) {
    const events =
      eventsToRender || this.filteredEvents.slice(0, this.currentPage * 12);
    ...
    try {
      if (
        window.ScrollReveal &&
        typeof window.ScrollReveal.refresh === "function"
      ) {
        window.ScrollReveal.refresh();
      } else if (
        window.ScrollReveal &&
        typeof window.ScrollReveal.init === "function"
      ) {
        window.ScrollReveal.init();
      }
    } catch (e) {
      console.debug(
        "[EventiManager] Errore chiamando ScrollReveal.refresh()",
        e
      );
    }
    ...
    if (this.loadMoreBtn) {
      if (
        this.hasMoreEvents &&
        this.filteredEvents.length > this.currentPage * 12
      ) {
        this.loadMoreBtn.classList.remove("d-none");
      } else {
        this.loadMoreBtn.classList.add("d-none");
  ...
  createEventCard(event, template, index) {
    ...
    const strippedDesc = event.descrizione
      ? this.stripHtml(event.descrizione)
      : "";
    description.textContent =
      strippedDesc.substring(0, 120) + (strippedDesc.length > 120 ? "..." : "");
  ...
  showError(message) {
    ...
    try {
      if (
        window.AdminGlobal &&
        window.AdminGlobal.ToastManager &&
        typeof window.AdminGlobal.ToastManager.error === "function"
      ) {
      ...
      if (
        ...
        typeof toastManager.error === "function"
      ) {
