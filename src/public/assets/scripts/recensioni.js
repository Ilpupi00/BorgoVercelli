class Reviews {
  ...
  renderReviews() {
    ...
    filtered.slice(0, this.shownReviews).forEach((review, index) => {
      ...
      const hasImage =
        review.immagine_utente &&
        review.immagine_utente.trim() &&
        review.immagine_utente !== "null";
      const avatarHtml = hasImage
        ? `<img src="${
            review.immagine_utente
          }" alt="Foto profilo" class="review-avatar-img" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                <div class="review-avatar" style="display:none;">${(review.nome_utente &&
                review.cognome_utente
                  ? review.nome_utente.charAt(0) +
                    review.cognome_utente.charAt(0)
                  : "U"
                ).toUpperCase()}</div>`
        : `<div class="review-avatar">${(review.nome_utente &&
          review.cognome_utente
            ? review.nome_utente.charAt(0) + review.cognome_utente.charAt(0)
            : "U"
          ).toUpperCase()}</div>`;
    ...
    if (this.loadMoreBtn) {
      this.loadMoreBtn.style.display =
        filtered.length > this.shownReviews ? "inline-block" : "none";
    }
  }
}
