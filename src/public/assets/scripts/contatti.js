class ContactForm {
  ...
  setupForm(form, formId) {
    ...
    form.addEventListener("submit", (event) =>
      this.handleSubmit(event, form, submitBtn, formId)
    );
  ...
  async handleSubmit(event, form, submitBtn, formId) {
    ...
    try {
      ...
      if (response.ok && result && result.success) {
        ...
        const errMsg =
          result && (result.error || (result.details && result.details.message))
        ...
        if (
          ...
          try {
            window.ShowModal.showModalError(errMsg, "Errore invio messaggio");
          } catch (e) {
            /* ignore */
          }
        } else {
