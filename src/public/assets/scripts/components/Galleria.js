...
class Galleria {
    ...
    try {
      ...
      if (this.allImages.length > 0) {
        ...
        if (headerImg) {
          ...
          headerImg.alt =
            this.allImages[0].descrizione || "Immagine della galleria";
        }
      }
  ...
  showNextImages() {
    const nextImages = this.allImages.slice(
      this.imagesShown,
      this.imagesShown + this.imagesPerPage
    );
        ...
        try {
          ...
          if (response.ok) {
            ShowModal.showModalSuccess(
              "Foto caricata",
              "La foto è stata caricata con successo!"
            );
          ...
          ShowModal.showModalError(
            "Errore di rete",
            "Errore durante il caricamento della foto. Riprova."
          );
        } finally {
          // Ripristina pulsante
  ...
  setupReturnToGallery(imageIndex) {
    ...
    returnBtn.innerHTML =
      '<i class="bi bi-arrow-down"></i> Torna alla Galleria';
    ...
    returnBtn.addEventListener("click", () => {
      ...
      const galleryItem = this.page.querySelector(
