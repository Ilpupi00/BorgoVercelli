...
class Squadre {
  ...
  async fetchSquadre() {
    try {
      ...
      if (!response.ok) {
        console.error(
          "Errore nel recupero delle squadre:",
          response.statusText
        );
        return [];
      }
  ...
  setupComponents(squadre) {
    ...
    squadreAnno = squadre.filter(
      (s) => String(s.Anno) === String(annoSelect.value)
    );
  ...
  handleSquadraChange(
    ...
    const selectedSquadra = squadre.find(
  ...
  handleAnnoChange(
    ...
    const filteredSquadre = squadre.filter(
      (s) => String(s.Anno) === String(selectedAnno)
    );
