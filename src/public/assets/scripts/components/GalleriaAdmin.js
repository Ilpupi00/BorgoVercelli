class GalleriaAdmin {
    ...
    fileInput.addEventListener(
      ...
      async (ev) => {
        ...
        if (
          ...
          typeof ShowModal.showUploadModal === "function"
        ) {
          try {
            ...
            // Fallback: prompt
            const descrizione =
              prompt("Inserisci descrizione (opzionale):") || "";
            await this.uploadFile(file, descrizione);
          }
        } else {
          const descrizione =
      ...
      if (res.ok) {
        ...
        if (typeof ShowModal !== "undefined" && ShowModal.showModalSuccess) {
          await ShowModal.showModalSuccess(
            ...
            data.message || "Immagine caricata con successo!"
          );
          ...
          if (successModal && typeof bootstrap !== "undefined") {
            ...
            if (imageObj)
              setTimeout(() => this.addImageToGallery(imageObj), 300);
          }
        } else {
        ...
        const errMsg =
          data.error || data.message || "Errore durante il caricamento";
        if (typeof ShowModal !== "undefined" && ShowModal.showModalError)
          await ShowModal.showModalError(errMsg, "Errore");
        else alert(errMsg);
      }
    } catch (err) {
  ...
  async performDelete() {
    if (!this.currentImageId) {
      if (typeof ShowModal !== "undefined")
        await ShowModal.showModalError("ID immagine non trovato", "Errore");
      else alert("Errore: ID immagine non trovato");
      return;
    }

    try {
      const res = await fetch(`/DeleteImmagine/${this.currentImageId}`, {
        method: "DELETE",
        credentials: "include",
      });
      ...
      if (res.ok && (data.message || data.success)) {
        if (typeof ShowModal !== "undefined") {
          ...
          if (successModal && typeof bootstrap !== "undefined") {
            ...
            successModal.addEventListener("hidden.bs.modal", onHidden, {
              once: true,
            });
          } else {
  ...
  addImageToGallery(image) {
    ...
    const descr = image.descrizione
      ? image.descrizione.replace(/"/g, "&quot;")
      : "";
    const created = image.created_at
        ...
        <div class="card-body d-flex flex-column">
          ...
          <div class="mt-auto">
            <div class="btn-group w-100" role="group">
              <button class="btn btn-sm btn-outline-primary" onclick="galleriaAdmin.visualizzaImmagine('${
                image.url
              }')"><i class="bi bi-eye"></i></button>
              <button class="btn btn-sm btn-outline-warning modifica-btn" data-id="${
                image.id || ""
              }" data-descrizione="${descr}"><i class="bi bi-pencil"></i></button>
              <button class="btn btn-sm btn-outline-danger elimina-btn" data-id="${
                image.id || ""
    ...
    if (modificaBtn)
      modificaBtn.addEventListener("click", (e) =>
        this.modificaImmagine(
          e.currentTarget.dataset.id,
          e.currentTarget.dataset.descrizione
        )
      );
    const eliminaBtn = col.querySelector(".elimina-btn");
    if (eliminaBtn)
      eliminaBtn.addEventListener("click", (e) =>
        this.eliminaImmagine(e.currentTarget.dataset.id)
      );
  }
}
