class GestionePrenotazione {
    ...
    document.addEventListener("click", (e) => {
      ...
      if (!btn) {
        if (e.target.id === "deleteScaduteBtn") {
          console.log(
            "[GestionePrenotazione] click on non-button element with id deleteScaduteBtn"
          );
          this.deleteScadute();
        }
        return;
      ...
      // Debug info
      console.log("[GestionePrenotazione] button clicked:", {
        id: btn.id,
    ...
    rows.forEach((row) => {
      ...
      if (statusCell) {
        ...
        const rowStatus = badge
          ? badge.textContent.toLowerCase().replace(" ", "_")
          : "";
  ...
  async visualizzaPrenotazione(id) {
    try {
      ...
      if (
        window.ShowModal &&
        typeof window.ShowModal.showModalInfo === "function"
      ) {
        // showModalInfo expects (msg, string) where string is title
        await window.ShowModal.showModalInfo(
          this.createDetailsHTML(prenotazione),
      ...
      if (
        window.ShowModal &&
        typeof window.ShowModal.showModalError === "function"
      ) {
        window.ShowModal.showModalError(
  ...
  async confermaPrenotazione(id) {
    if (
      window.ShowModal &&
      typeof window.ShowModal.modalConfirm === "function"
    ) {
      ...
      try {
        ...
        if (result.success) {
          if (
            window.ShowModal &&
            typeof window.ShowModal.showModalSuccess === "function"
          ) {
      ...
      if (confirm("Confermare questa prenotazione?")) {
        try {
          const response = await fetch(
            `/prenotazione/prenotazioni/${id}/stato`,
            {
              method: "PATCH",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ stato: "confermata" }),
        ...
        if (result.success) {
          if (
            ...
            window.ShowModal.showModalSuccess(
              ...
              "La prenotazione è stata riattivata con successo"
            );
          }
          this.refreshTable();
        } else if (
  ...
  async modificaPrenotazione(id) {
    try {
      ...
      this.showModal(
        ...
        this.createEditHTML(prenotazione),
        true,
        async () => {
  ...
  async eliminaPrenotazione(id) {
    if (
      ...
      const confirmed = await window.ShowModal.modalDelete(
        "Sei sicuro di voler eliminare questa prenotazione? Questa operazione è irreversibile.",
  ...
  async deleteScadute() {
    if (
      ...
      try {
        console.log(
          "[GestionePrenotazione] deleteScadute: calling check-scadute"
        );
        try {
          const checkResp = await fetch(
            "/prenotazione/prenotazioni/check-scadute",
            { method: "POST" }
          );
          console.log(
            "[GestionePrenotazione] check-scadute status:",
            checkResp.status
          );
          const checkJson = await checkResp.json().catch((e) => {
            console.warn("check-scadute json parse failed", e);
            return null;
          });
          console.log(
            "[GestionePrenotazione] check-scadute response json:",
            checkJson
          );
        } catch (e) {
        ...
        console.log(
          "[GestionePrenotazione] deleteScadute: calling DELETE /prenotazione/prenotazioni/scadute"
        );
        const response = await fetch("/prenotazione/prenotazioni/scadute", {
          method: "DELETE",
        });
        console.log(
          "[GestionePrenotazione] DELETE scadute status:",
          response.status
        );
        const result = await response.json().catch((e) => {
          console.warn("delete scadute json parse failed", e);
          return null;
        });
        ...
        if (result && result.success) {
          ...
          console.warn(
            "[GestionePrenotazione] deleteScadute did not return success:",
            result
          );
          if (
            window.ShowModal &&
        ...
        console.error(
          "[GestionePrenotazione] deleteScadute handler error",
          error
        );
        if (
          window.ShowModal &&
          ...
          window.ShowModal.showModalError(
            "Errore nell'eliminazione delle prenotazioni",
            "Errore"
          );
        } else {
          alert("Errore nell'eliminazione");
      ...
      if (confirm("Eliminare tutte le prenotazioni scadute?")) {
        try {
          try {
            await fetch("/prenotazione/prenotazioni/check-scadute", {
              method: "POST",
            });
          } catch (e) {
            console.warn("check-scadute fallito", e);
          }
          const response = await fetch("/prenotazione/prenotazioni/scadute", {
            method: "DELETE",
          });
  ...
  createDetailsHTML(prenotazione) {
    return `
            ...
            <p><strong>Utente:</strong> ${prenotazione.utente_nome} ${
      prenotazione.utente_cognome
    }</p>
            <p><strong>Squadra:</strong> ${prenotazione.squadra_nome || "-"}</p>
            <p><strong>Data:</strong> ${new Date(
              prenotazione.data_prenotazione
            ).toLocaleDateString("it-IT")}</p>
            <p><strong>Orario:</strong> ${prenotazione.ora_inizio} - ${
    ...
    return `
            <form id="editForm">
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="campo_id" name="campo_id" value="${
                      prenotazione.campo_id
                    }" required>
                </div>
                <div class="mb-3">
                    <label for="data_prenotazione" class="form-label">Data</label>
                    <input type="date" class="form-control" id="data_prenotazione" name="data_prenotazione" value="${
                      prenotazione.data_prenotazione
                    }" required>
                </div>
                <div class="mb-3">
                    <label for="ora_inizio" class="form-label">Ora Inizio</label>
                    <input type="time" class="form-control" id="ora_inizio" name="ora_inizio" value="${
                      prenotazione.ora_inizio
                    }" required>
                </div>
                <div class="mb-3">
                    <label for="ora_fine" class="form-label">Ora Fine</label>
                    <input type="time" class="form-control" id="ora_fine" name="ora_fine" value="${
                      prenotazione.ora_fine
                    }" required>
                </div>
                <div class="mb-3">
                    <label for="tipo_attivita" class="form-label">Tipo Attività</label>
                    <input type="text" class="form-control" id="tipo_attivita" name="tipo_attivita" value="${
                      prenotazione.tipo_attivita || ""
                    }">
                </div>
                <div class="mb-3">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            ${
                              hasSave
                                ? '<button type="button" class="btn btn-primary" id="saveBtn">Salva</button>'
                                : ""
                            }
                        </div>
                    </div>
