...
class GestioneCampionati {
    ...
    row.innerHTML = `
            <td>
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <strong class="d-block">${this.escapeHtml(
                          championship.nome
                        )}</strong>
            ...
            <td>
                <span class="badge bg-info bg-gradient">${this.escapeHtml(
                  categoryLabel
                )}</span>
            </td>
  ...
  async handleDeleteChampionship() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/campionati/${this.currentChampionship.id}`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      const modal = bootstrap.Modal.getInstance(
        document.getElementById("deleteChampionshipModal")
      );
      if (modal) modal.hide();
  ...
  handleSearch() {
    ...
    if (searchTerm === "") {
      ...
      this.filteredChampionships = this.championships.filter(
        (championship) =>
          ...
          (championship.categoria &&
            championship.categoria.toLowerCase().includes(searchTerm))
      );
    }
  ...
  handleFilter() {
    ...
    this.filteredChampionships = this.championships.filter((championship) => {
      ...
      const categoryMatch =
        !categoryFilter || championship.categoria === categoryFilter;
  ...
  updateStats() {
    ...
    const totalTeams = this.championships.reduce(
      (sum, c) => sum + (c.numero_squadre || 0),
      0
    );
    const upcomingMatches = this.championships.reduce(
      (sum, c) => sum + (c.partite_programmate || 0),
      0
    );
  ...
  getStatusBadge(status) {
    const badges = {
      attivo:
        '<span class="badge bg-success bg-gradient"><i class="bi bi-check-circle me-1"></i>Attivo</span>',
      inattivo:
        '<span class="badge bg-secondary bg-gradient"><i class="bi bi-pause-circle me-1"></i>Inattivo</span>',
    };
    return (
      badges[status] || '<span class="badge bg-secondary">Sconosciuto</span>'
    );
  }
