  ...
  _setup() {
    ...
    try {
      ...
      const currentPrimary =
        (cs.getPropertyValue("--primary-color") || "").trim() || "#0d6efd";
  ...
  attachListeners() {
    ...
    if (lightBtn && darkBtn && autoBtn && window.themeManager) {
      lightBtn.addEventListener("click", (e) => {
        e.preventDefault();
        window.themeManager.applyTheme(window.themeManager.themes.LIGHT);
      });
      darkBtn.addEventListener("click", (e) => {
        e.preventDefault();
        window.themeManager.applyTheme(window.themeManager.themes.DARK);
      });
      autoBtn.addEventListener("click", (e) => {
        e.preventDefault();
        window.themeManager.applyTheme(window.themeManager.themes.AUTO);
      });
    }
  }
}
