class GestioneEventi {
  ...
  initialize() {
    ...
    console.log(
      "Gestione eventi caricata con",
      this.originalEventi.length,
      "eventi"
    );
  }
  ...
  loadEventiData() {
    const rows = document.querySelectorAll(
      "#eventiTableBody tr[data-evento-id]"
    );
  ...
  setupEventListeners() {
    ...
    document.querySelectorAll(".toggle-pubblicazione").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        const eventoId = e.currentTarget.dataset.eventoId;
        const currentStatus =
          parseInt(e.currentTarget.dataset.currentStatus) === 1;
  ...
  filterEventi() {
    ...
    this.originalEventi.forEach((evento) => {
      ...
      if (
        searchTerm &&
        !(
          evento.titolo.includes(searchTerm) ||
  ...
  async eliminaEvento(id) {
        ...
        .then(async (confirmed) => {
          ...
          try {
            ...
            if (response.ok && result.success) {
              ...
              this.originalEventi = this.originalEventi.filter(
                (e) => e.id !== id
              );
              ...
              // Aggiorna i contatori
              this.updateCounters(
                this.originalEventi.filter(
                  (e) => e.element.style.display !== "none"
                ).length
              );
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        // Aggiorna i contatori
        this.updateCounters(
          this.originalEventi.filter((e) => e.element.style.display !== "none")
            .length
        );
  ...
  async togglePubblicazione(id, currentStatus) {
    ...
    if (typeof ShowModal !== "undefined" && typeof bootstrap !== "undefined") {
      ShowModal.modalConfirm(
        `Sei sicuro di voler ${action} questo evento?`,
        "Conferma",
        currentStatus ? "Sospendi" : "Pubblica",
        currentStatus ? "btn-warning" : "btn-success",
        currentStatus ? "bi-eye-slash" : "bi-eye"
      )
        .then(async (confirmed) => {
          if (!confirmed) return;
          try {
            ...
            if (response.ok && result.success) {
              ...
              const toggleBtn = document.querySelector(
                `tr[data-evento-id="${id}"] .toggle-pubblicazione`
              );
              if (toggleBtn) {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        this.showAlert(
          "Errore nell'operazione: " + (result.error || "Errore sconosciuto"),
          "danger"
        );
      }
    } catch (error) {
