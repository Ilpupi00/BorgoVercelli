...
class Prenotazione {
      ...
      btn.addEventListener("click", async (e) => {
        ...
        try {
          ...
          if (resUser.ok) {
            ...
            if (
              ShowModal &&
              typeof ShowModal.showLoginRequiredModal === "function"
            ) {
              ShowModal.showLoginRequiredModal(
          ...
          if (
            ...
            ShowModal.showLoginRequiredModal(
              "Errore nel controllo del login. Riprova."
            );
          }
        }
      });
  ...
  async updateOrariDisponibili(campoId, data) {
    try {
      ...
      const res = await fetch(
        `/prenotazione/campi/${campoId}/disponibilita?data=${data}`
      );
      const orari = await res.json();
      const now = new Date();
      let filteredOrari = Array.isArray(orari)
        ? orari.filter((o) => {
      ...
      if (filteredOrari.length === 0) {
        ...
        try {
          const res2 = await fetch(
            `/prenotazione/campi/${campoId}/disponibilita?data=${tStr}`
          );
          const orari2 = await res2.json();
          filteredOrari = Array.isArray(orari2)
            ? orari2.filter((o) => {
                if (typeof o.prenotato !== "undefined" && o.prenotato)
                  return false;
                return true;
              })
            : [];
          if (filteredOrari.length > 0) {
            resultDiv.innerHTML =
              `<div class="text-muted small mb-2">Nessun orario disponibile per ${data}. Mostrati gli orari per ${tStr}:</div>` +
              filteredOrari
                .map(
                  (o) =>
      ...
      resultDiv.innerHTML = filteredOrari
        .map(
          (o) =>
            `<span class="badge bg-success text-light border p-2">${o.inizio}-${o.fine}</span>`
        )
  ...
  async openPrenotaModal(campoId) {
    ...
    const inputData =
      this.page.querySelector(`.input-orari-campo[data-campo-id='${campoId}']`)
    ...
    try {
      const res = await fetch(
        `/prenotazione/campi/${campoId}/disponibilita?data=${inputData}`
      );
      const orari = await res.json();
      const now = new Date();
      ...
      const { showModalPrenotazione } = await import(
        "../utils/modalPrenotazione.js"
      );
      showModalPrenotazione(
        campo,
        filteredOrari,
        inputData,
        async (datiPrenotazione) => {
          let utenteId = null;
          try {
            const resUser = await fetch("/session/user");
            if (resUser.ok) {
          ...
          try {
            console.log("[PRENOTAZIONE] Invio dati:", {
              ...datiPrenotazione,
              utente_id: utenteId,
            });
            ...
            if (result && result.success) {
              if (ShowModal && typeof ShowModal.showModalInfo === "function") {
                ShowModal.showModalInfo(
                  "La tua prenotazione è stata inviata e è in attesa di approvazione.",
              ...
              const data =
                this.page.querySelector(
                  `.input-orari-campo[data-campo-id='${campoId}']`
                )?.value || new Date().toISOString().slice(0, 10);
              await this.updateOrariDisponibili(campoId, data);
            } else {
              if (ShowModal && typeof ShowModal.showModalError === "function") {
                ShowModal.showModalError(
                  result.error || "Errore nella prenotazione"
                );
              }
            }
          } catch (err) {
