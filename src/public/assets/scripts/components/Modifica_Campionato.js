class ModificaCampionato {
  ...
  bindEvents() {
    ...
    const addTeamModal = new bootstrap.Modal(
  ...
  async loadTeams() {
    try {
      const response = await fetch(
        `/api/admin/campionati/${this.campionatoId}/squadre`,
        {
          credentials: "include",
        }
      );
      ...
      .map(
        (team, index) => `
            <tr>
                ...
                <td><strong>${
                  team.nome || team.squadra_nome || "N/A"
                }</strong></td>
                <td>
                    <input type="number" class="form-control form-control-sm" 
                           value="${team.punti || 0}" 
  ...
  async addNewTeam() {
    ...
    if (
      this.teams.some((t) => (t.nome || t.squadra_nome) === squadraNome.trim())
    ) {
    ...
    const teamData = {
      ...
      posizione:
        parseInt(document.getElementById("posizioneInput").value) ||
        this.teams.length + 1,
      ...
      pareggiate:
        parseInt(document.getElementById("pareggiteInput").value) || 0,
      gol_fatti: parseInt(document.getElementById("golFattiInput").value) || 0,
      gol_subiti:
        parseInt(document.getElementById("golSubitiInput").value) || 0,
    };
    ...
    try {
      const response = await fetch(
        `/api/admin/campionati/${
          this.campionatoId
        }/squadre/${encodeURIComponent(nomeSquadra)}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  async deleteTeam(squadraNome) {
    if (
      !confirm(`Sei sicuro di voler rimuovere "${squadraNome}" dal campionato?`)
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/admin/campionati/${
          this.campionatoId
        }/squadre/${encodeURIComponent(squadraNome)}`,
        {
          method: "DELETE",
          credentials: "include",
        }
  ...
  async handleSubmit(e) {
    ...
    const championshipData = {
      ...
      retrocessione_diretta:
        parseInt(formData.get("retrocessione_diretta")) || 2,
    };
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/campionati/${this.campionatoId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
