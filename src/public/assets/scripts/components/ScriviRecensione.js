...
class ScriviRecensione {
  ...
  setupFormSubmission() {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      const privacyCheckbox = this.page.querySelector(
        "#recensione_privacy_accept"
      );
      ...
      if (!privacyCheckbox.checked) {
        ...
        ShowModal.showModalError(
          "Devi accettare la Privacy Policy per inviare la recensione.",
          "Privacy Policy Richiesta"
        );
        return;
      }
      ...
      try {
        ...
        if (result.success) {
          ShowModal.showModalSuccess(
            ...
            "Grazie per il tuo feedback, la tua recensione è stata salvata."
          );
          form.reset();
          this.setRating(0);
          ...
          ShowModal.showModalError(
            result.error || "Errore nell'invio della recensione"
          );
        }
      } catch (error) {
