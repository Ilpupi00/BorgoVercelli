class GestioneRecensioni {
  ...
  async visualizzaRecensione(id) {
    try {
      ...
      document.getElementById(
        "modalRecensioneUtente"
      ).textContent = `${recensione.utente_nome} ${recensione.utente_cognome}`;
      document.getElementById("modalRecensioneValutazione").innerHTML =
        this.renderStars(recensione.valutazione);
      document.getElementById("modalRecensioneTitolo").textContent =
        recensione.titolo;
      document.getElementById("modalRecensioneContenuto").textContent =
        recensione.contenuto;
      document.getElementById("modalRecensioneData").textContent = new Date(
      ...
      this.mostraErrore(
        "Errore nel caricamento della recensione: " + error.message
      );
    }
  }
  ...
  async toggleVisibile(id, currentStatus) {
    ...
    if (typeof ShowModal !== "undefined" && typeof bootstrap !== "undefined") {
      ...
      ShowModal.modalConfirm(
        `Sei sicuro di voler ${action} questa recensione?`,
        "Conferma",
        confirmLabel,
        confirmClass,
        iconClass,
        newStatus ? "text-success" : "text-warning"
      );
        ...
        try {
          ...
          if (typeof ShowModal !== "undefined" && ShowModal.showModalSuccess) {
            await ShowModal.showModalSuccess(
              ...
              `Recensione ${newStatus ? "mostrata" : "nascosta"} con successo`
            );
          } else {
            this.mostraSuccesso(
              `Recensione ${newStatus ? "mostrata" : "nascosta"} con successo`
  ...
  async eliminaRecensione(id) {
    ...
    if (typeof ShowModal !== "undefined" && typeof bootstrap !== "undefined") {
      ShowModal.modalDelete(
        "Sei sicuro di voler eliminare definitivamente questa recensione? Questa azione non può essere annullata.",
  ...
  updateRowVisibility(id, visibile) {
    const row = document.querySelector(
      `#recensioniTableBody tr[data-recensione-id="${id}"]`
    );
    if (!row) return;
    ...
    if (azioniCell) {
      ...
      const toggleBtn = azioniCell.querySelector(
        'button[title="Nascondi"], button[title="Mostra"]'
      );
      if (toggleBtn) {
        // Crea nuovo bottone con stato opposto
        const newBtn = document.createElement("button");
        newBtn.className = visibile
          ? "btn btn-sm btn-outline-warning"
          : "btn btn-sm btn-outline-success";
        newBtn.title = visibile ? "Nascondi" : "Mostra";
        newBtn.innerHTML = visibile
  ...
  removeRow(id) {
    ...
    try {
      ...
      const totalCount = document.querySelectorAll(
        "#recensioniTableBody tr[data-recensione-id]"
      ).length;
      const visibleCount = document.querySelectorAll(
        '#recensioniTableBody tr[data-recensione-id]:not([style*="display: none"])'
      ).length;
...
window.visualizzaRecensione = (id) =>
  gestioneRecensioni.visualizzaRecensione(id);
...
window.mostraRecensioniVisibili = () =>
  gestioneRecensioni.mostraRecensioniVisibili();
window.mostraRecensioniNascoste = () =>
  gestioneRecensioni.mostraRecensioniNascoste();
