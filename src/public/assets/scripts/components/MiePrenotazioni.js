class MiePrenotazioni {
  ...
  filterPrenotazioni() {
    ...
    this.filteredPrenotazioni = this.prenotazioni.filter((prenotazione) => {
      ...
      const statusMatch =
        this.currentFilter === "all" ||
        prenotazione.stato === this.currentFilter;

      // Filter by search term
      const searchMatch =
        !searchTerm ||
        (prenotazione.campo_nome &&
          prenotazione.campo_nome.toLowerCase().includes(searchTerm)) ||
        (prenotazione.data_prenotazione &&
          prenotazione.data_prenotazione.includes(searchTerm)) ||
        (prenotazione.ora_inizio &&
          prenotazione.ora_inizio.includes(searchTerm)) ||
        (prenotazione.ora_fine && prenotazione.ora_fine.includes(searchTerm)) ||
        (prenotazione.tipo_attivita &&
          prenotazione.tipo_attivita.toLowerCase().includes(searchTerm));
  ...
  renderPrenotazioni() {
    ...
    this.emptyState.style.display = "none";
    this.prenotazioniCards.innerHTML = this.filteredPrenotazioni
      .map((prenotazione) => this.createPrenotazioneCard(prenotazione))
      .join("");
                ...
                <div class="prenotazione-card ${statusClass} animate__animated animate__fadeIn">
                    ...
                    <footer class="card-footer-custom">
                        <small class="text-muted">
                            ...
                            <time datetime="${
                              prenotazione.created_at
                            }">Prenotata il ${new Date(
      prenotazione.created_at
    ).toLocaleDateString("it-IT")}</time>
                        </small>
                    </footer>
                </div>
            </article>
  ...
  getStatusBadge(stato) {
    const badges = {
      confermata:
        '<span class="status-badge status-confermata"><i class="bi bi-check-circle-fill me-1"></i>Confermata</span>',
      in_attesa:
        '<span class="status-badge status-attesa"><i class="bi bi-clock-fill me-1"></i>In Attesa</span>',
      annullata:
        '<span class="status-badge status-annullata"><i class="bi bi-x-circle-fill me-1"></i>Annullata</span>',
      scaduta:
        '<span class="status-badge status-scaduta"><i class="bi bi-calendar-x-fill me-1"></i>Scaduta</span>',
    };
  ...
  attachCardEventListeners() {
    ...
    document.querySelectorAll('[data-action="cancel"]').forEach((btn) => {
      btn.addEventListener("click", () =>
        this.cancelPrenotazione(btn.dataset.id)
      );
    });
    ...
    document.querySelectorAll('[data-action="reactivate"]').forEach((btn) => {
      btn.addEventListener("click", () =>
        this.reactivatePrenotazione(btn.dataset.id)
      );
    });
    ...
    document.querySelectorAll('[data-action="delete"]').forEach((btn) => {
      btn.addEventListener("click", () =>
        this.deletePrenotazione(btn.dataset.id)
      );
    });
  }
    ...
    const detailsHTML = `
            <div class="prenotazione-details">
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <p class="mb-0">${prenotazione.ora_inizio} - ${
      prenotazione.ora_fine
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <p class="mb-0">${this.getStatusBadge(
                          prenotazione.stato
                        )}</p>
                    </div>
                    <div class="col-md-6 mb-3">
      ...
      try {
        ...
        if (response.ok && result.success) {
          ...
          const errorMessage =
            result.message ||
            result.error ||
            "Impossibile riattivare la prenotazione";
          if (
            window.ShowModal &&
            typeof window.ShowModal.showModalError === "function"
          ) {
        ...
        if (
          ...
          window.ShowModal.showModalError(
            "Errore durante la riattivazione della prenotazione",
            "Errore"
          );
        }
      }
        ...
        if (result.success) {
          if (
            ...
            window.ShowModal.showModalSuccess(
              ...
              "La prenotazione è stata eliminata definitivamente"
            );
          }
          await this.loadPrenotazioni();
        } else {
