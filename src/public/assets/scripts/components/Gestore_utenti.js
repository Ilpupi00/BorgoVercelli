class GestoreUtente {
  ...
  loadUtentiData() {
    const rows = document.querySelectorAll(
      "#utentiTableBody tr[data-utente-id]"
    );
  ...
  static async visualizzaUtente(id) {
    try {
      ...
      if (prenotazioni && prenotazioni.length > 0) {
        prenotazioniHTML = `
                    <div class="mt-4">
                        <h5><i class="bi bi-calendar-check me-2"></i>Prenotazioni (${
                          prenotazioni.length
                        })</h5>
                                    ...
                                    ${prenotazioni
                                      .map((p) => {
                                        ...
                                        const statoBadge =
                                          ...
                                          `<span class="badge bg-secondary">${stato}</span>`;
                                        ...
                                        return `
                                          ...
                                          p.ora_fine
                                                ...
                                                <td>${documentoInfo}</td>
                                                <td>
                                                    <button class="btn btn-sm btn-outline-primary" onclick="GestoreUtente.modificaPrenotazioneAdmin(${
                                                      p.id
                                                    })" title="Modifica">
      ...
      if (utente.stato === "bannato") {
        statoHTML = `
                    <div class="alert alert-danger mb-3">
                        ...
                        <p class="mb-1"><strong>Motivo:</strong> ${
                          utente.motivo_sospensione || "Non specificato"
                        }</p>
                        ${
                          utente.data_inizio_sospensione
                            ? `<p class="mb-0"><small>Data ban: ${new Date(
                                utente.data_inizio_sospensione
        ...
        utente.data_fine_sospensione &&
        new Date(utente.data_fine_sospensione) > new Date()
      ) {
        ...
        statoHTML = `
                    <div class="alert alert-warning mb-3">
                        ...
                        <p class="mb-1"><strong>Fino al:</strong> ${dataFine.toLocaleDateString(
                          "it-IT"
                        )} ${dataFine.toLocaleTimeString("it-IT", {
          hour: "2-digit",
          minute: "2-digit",
        })}</p>
                        ${
                          utente.data_inizio_sospensione
                            ? `<p class="mb-0"><small>Data inizio: ${new Date(
                                utente.data_inizio_sospensione
                              ).toLocaleDateString("it-IT")}</small></p>`
                            : ""
                        }
      ...
      const differenzaGiorni = Math.floor(
        (oggi - dataRegistrazione) / (1000 * 60 * 60 * 24)
      );
      let durataAccount = "";
      ...
      const content = `
                ...
                <div class="row mb-4 align-items-start">
                    <div class="col-md-8 mb-3 mb-md-0">
                        <h4 class="mb-3"><i class="bi bi-person-circle me-2"></i>${
                          utente.nome
                        } ${utente.cognome}</h4>
                        <div class="row g-3">
                            <div class="col-12 col-sm-6">
                                <div class="d-flex flex-column p-2 bg-light rounded">
                                    ...
                                    <span class="ms-4 text-break small">${
                                      utente.email
                                    }</span>
                                </div>
                            </div>
                            ...
                            <div class="col-12 col-sm-6">
                                <div class="d-flex flex-column p-2 bg-light rounded">
                                    ...
                                    <span class="ms-4 small">${GestoreUtente.getTipoUtenteLabel(
                                      utente.tipo_utente_id
                                    )}</span>
                                </div>
                            </div>
                            <div class="col-12 col-sm-6">
                            ...
                            <div class="col-12 col-sm-6">
                                <div class="d-flex flex-column p-2 bg-light rounded">
                                    ...
                                    <span class="ms-4 small">${dataRegistrazione.toLocaleDateString(
                                      "it-IT"
                                    )} <span class="text-muted">(${durataAccount})</span></span>
                                </div>
                            </div>
                            <div class="col-12 col-sm-6">
                                <div class="d-flex flex-column p-2 bg-light rounded">
                            ...
                            <div class="col-12 col-sm-6">
                                <div class="d-flex flex-column p-2 bg-light rounded">
                                    ...
                                    <span class="ms-4 small">${new Date(
                                      utente.data_nascita
                    ...
                    <div class="col-md-4 text-center d-flex flex-column align-items-center">
                        <img src="${
                          utente.immagine_profilo || "/assets/images/Logo.png"
                        }" 
                             alt="Foto profilo" 
      ...
      const modal = new bootstrap.Modal(
        document.getElementById("visualizzaModal")
      );
      modal.show();
    } catch (error) {
      console.error("Errore:", error);
      GestoreUtente.showNotification(
        "Errore nel caricamento dei dettagli utente",
        "error"
      );
    }
  }
  ...
  static async modificaUtente(id) {
    try {
      ...
      const modal = new bootstrap.Modal(
        document.getElementById("modificaModal")
      );
      modal.show();
    } catch (error) {
      console.error("Errore:", error);
  ...
  static async eliminaUtente(id) {
    try {
      ...
      if (typeof ShowModal !== "undefined" && ShowModal.showModalSuccess) {
        await ShowModal.showModalSuccess(
        ...
        if (staticSuccessModalEl && typeof bootstrap !== "undefined") {
          ...
          bs.show();
          staticSuccessModalEl.addEventListener(
            "hidden.bs.modal",
            () => {
              location.reload();
            },
      ...
      console.error("Errore:", error);
      await ShowModal.showModalError(
        error.message || "Errore nell'eliminazione dell'utente",
        "Errore"
      );
    }
  }
  ...
  static mostraSospendiBan(id, nome, cognome) {
    ...
    console.log(
      "💾 Dati salvati in window.tempUtenteData:",
      window.tempUtenteData
    );
  ...
  static mostraSospensione() {
    ...
    setTimeout(() => {
      ...
      const sospensioneIdElement = document.getElementById(
        "sospensioneUtenteId"
      );
      const sospensioneNomeElement = document.getElementById(
        "sospensioneUtenteNome"
      );

      if (sospensioneIdElement) sospensioneIdElement.value = data.id;
      if (sospensioneNomeElement)
        sospensioneNomeElement.textContent = data.nomeCompleto;
  ...
  static mostraBan() {
    ...
    // Hide choice modal
    bootstrap.Modal.getInstance(
      document.getElementById("sceltaSospendiBanModal")
    ).hide();
  ...
  static tornaScelta() {
    // Hide current modal
    const sospensioneModal = bootstrap.Modal.getInstance(
      document.getElementById("sospensioneModal")
    );
    const banModal = bootstrap.Modal.getInstance(
  ...
  static async confermaSospensione() {
    ...
    try {
      ...
      if (typeof ShowModal !== "undefined" && ShowModal.showModalSuccess) {
        await ShowModal.showModalSuccess(
          "Operazione completata",
          "Utente sospeso con successo. Email di notifica inviata."
        );
        setTimeout(() => location.reload(), 1000);
      } else {
        GestoreUtente.showNotification(
          "Utente sospeso con successo",
          "success"
        );
  ...
  static async confermaBan() {
    ...
    try {
      ...
      if (typeof ShowModal !== "undefined" && ShowModal.showModalSuccess) {
        await ShowModal.showModalSuccess(
          "Operazione completata",
          "Utente bannato con successo. Email di notifica inviata."
        );
        setTimeout(() => location.reload(), 1000);
      } else {
        GestoreUtente.showNotification(
  ...
  static async confermaRevoca() {
    ...
    try {
      ...
      bootstrap.Modal.getInstance(
        document.getElementById("revocaModal")
      ).hide();

      if (typeof ShowModal !== "undefined" && ShowModal.showModalSuccess) {
        await ShowModal.showModalSuccess(
          "Operazione completata",
          "Sospensione/Ban revocato con successo. Email di notifica inviata."
        );
        setTimeout(() => location.reload(), 1000);
      } else {
        GestoreUtente.showNotification(
          "Revoca completata con successo",
          "success"
        );
        setTimeout(() => location.reload(), 1500);
      }
  ...
  static async modificaPrenotazioneAdmin(prenotazioneId) {
    try {
      ...
      const responsePrenotazione = await fetch(
        `/api/prenotazioni/${prenotazioneId}`
      );
      ...
      campoSelect.innerHTML = campi
        .map(
          (c) =>
            `<option value="${c.id}" ${
              c.id === prenotazione.campo_id ? "selected" : ""
            }>${c.nome}</option>`
        )
        .join("");
      ...
      document.getElementById("modPrenData").value =
        prenotazione.data_prenotazione.split("T")[0];
      document.getElementById("modPrenOraInizio").value =
      ...
      if (prenotazione.tipo_documento === "CF") {
        document.getElementById("modPrenCodiceFiscale").value =
          prenotazione.codice_fiscale || "";
      } else if (prenotazione.tipo_documento === "ID") {
        document.getElementById("modPrenNumeroDoc").value =
          prenotazione.numero_documento || "";
      }
  ...
  static async salvaModificaPrenotazioneAdmin() {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(
          error.error || "Errore nella modifica della prenotazione"
        );
      }
      ...
      GestoreUtente.showNotification(
        "Prenotazione modificata con successo. Utente notificato.",
        "success"
      );
