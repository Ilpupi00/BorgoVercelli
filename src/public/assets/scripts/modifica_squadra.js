...
class TeamManager {
  constructor(notificationManager, loadingManager) {
    ...
    this.teamId =
      document.getElementById("teamId")?.value || this.extractTeamIdFromUrl();
  ...
  async handleSubmit(e) {
    ...
    try {
      ...
      this.notificationManager.showError(
        error.message || "Errore durante l'aggiornamento della squadra"
      );
    } finally {
  ...
  async confirmDelete() {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        setTimeout(
          () => (window.location.href = "/admin/gestione-squadre"),
          1500
        );
      } else {
      ...
      console.error("Errore:", error);
      this.notificationManager.showError(
        error.message || "Errore durante l'eliminazione della squadra"
      );
    } finally {
      this.loadingManager.hide();
...
class PlayerManager {
  constructor(notificationManager, loadingManager) {
    ...
    this.playerModal = modalGiocatoreEl
      ? new bootstrap.Modal(modalGiocatoreEl)
      : null;
  ...
  init() {
    ...
    const newPlayerTrigger = document.querySelector(
      '[data-bs-target="#modalGiocatore"]'
    );
    if (newPlayerTrigger) {
    ...
    const playerPreviewContainer = document.getElementById(
    ...
    if (fotoInput) {
      fotoInput.addEventListener("change", (ev) => {
        ...
        if (f.size > 5 * 1024 * 1024) {
          this.notificationManager.showError(
            "Il file è troppo grande. Massimo 5MB"
          );
          ev.target.value = "";
          return;
        }
  ...
  handlePlayerAction(e) {
    ...
    const playerId =
      target.dataset.id || (playerCard ? playerCard.dataset.playerId : null);
  ...
  openEditModal(playerId) {
    ...
    if (document.getElementById("giocatoreNome"))
      document.getElementById("giocatoreNome").value = nome;
    if (document.getElementById("giocatoreCognome"))
      document.getElementById("giocatoreCognome").value = cognome;
    if (document.getElementById("giocatoreRuolo"))
      document.getElementById("giocatoreRuolo").value = ruolo;
    if (document.getElementById("numeroMaglia"))
      document.getElementById("numeroMaglia").value = numero;
    if (document.getElementById("dataNascita"))
  ...
  async removePlayer(playerId) {
    ...
    const ok = window.ShowModal
      ? await ShowModal.modalDelete(
          "Sei sicuro di voler rimuovere questo giocatore dalla squadra? Questa operazione è irreversibile.",
          "Conferma eliminazione"
        )
      : confirm(
          "Sei sicuro di voler rimuovere questo giocatore dalla squadra?"
        );
    ...
    try {
      const response = await fetch(
        `/squadre/${this.teamId}/giocatori/${playerId}`,
        {
          method: "DELETE",
          credentials: "same-origin",
        }
      ...
      if (response.ok && result.success) {
        ...
        throw new Error(
          result.error || "Errore durante la rimozione del giocatore"
        );
      }
    } catch (error) {
      console.error("Errore:", error);
      this.notificationManager.showError(
        error.message || "Errore durante la rimozione del giocatore"
      );
    } finally {
      this.loadingManager.hide();
    }
  ...
  async savePlayer() {
    ...
    const playerId =
      this.playerIdInput && this.playerIdInput.value
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        throw new Error(
          result.error || "Errore durante il salvataggio del giocatore"
        );
      }
    } catch (error) {
      console.error("Errore:", error);
      this.notificationManager.showError(
        error.message || "Errore durante il salvataggio del giocatore"
      );
    } finally {
      this.loadingManager.hide();
    }
...
class ManagerManager {
  constructor(notificationManager, loadingManager) {
    ...
    this.addManagerModal = modalDirigenteEl
      ? new bootstrap.Modal(modalDirigenteEl)
      : null;
  ...
  init() {
    if (this.managersList) {
      this.managersList.addEventListener("click", (e) =>
        this.handleManagerAction(e)
      );
    }
    if (this.managerSearch) {
      this.managerSearch.addEventListener("input", (e) =>
        this.handleSearchInput(e)
      );
      this.managerSearch.addEventListener("keydown", (e) =>
    ...
    if (deleteDirigenteBtn) {
      deleteDirigenteBtn.addEventListener("click", () =>
        this.deleteManagerFromModal()
      );
    }
    ...
    try {
      if (this.managerSearch) {
        ["focus", "mousedown", "click", "keydown", "input"].forEach((evt) => {
          this.managerSearch.addEventListener(
            evt,
            (e) => {
              console.debug(
                "[DEBUG] managerSearch event:",
                evt,
                "disabled=",
                this.managerSearch.disabled,
                "readonly=",
                this.managerSearch.readOnly,
                "value=",
      ...
      if (this.addManagerModal) {
        this.addManagerModal._element.addEventListener("shown.bs.modal", () => {
          setTimeout(() => {
            if (this.managerSearch) {
              ...
              console.debug(
                "[DEBUG] managerSearch computed pointer-events:",
                cs.pointerEvents,
                "visibility:",
                cs.visibility,
                "opacity:",
            ...
            if (modalEl && inputEl && inputEl.getBoundingClientRect) {
              ...
              console.debug(
                "[DEBUG] elementFromPoint at search coords:",
                topEl
              );
              ...
              try {
                if (
                  topEl &&
                  topEl !== inputEl &&
                  !inputEl.contains(topEl) &&
                  topEl.style
                ) {
                  console.debug(
                    "[DEBUG] Temporarily disabling pointer-events on overlay element:",
                    topEl
                  );
                  // store previous value
                  this._tempDisabledOverlays.push({
                    el: topEl,
                    prev: topEl.style.pointerEvents,
                  });
                  topEl.style.pointerEvents = "none";
                }
              } catch (err) {
                console.warn(
                  "Could not modify overlay element pointer-events:",
                  err
                );
              }
            }
        ...
        // restore on hide
        this.addManagerModal._element.addEventListener(
          "hidden.bs.modal",
          () => {
            try {
              this._tempDisabledOverlays.forEach((item) => {
                if (item && item.el && item.el.style)
                  item.el.style.pointerEvents = item.prev || "";
              });
            } catch (err) {
  ...
  handleManagerAction(e) {
    ...
    if (managerCard && managerCard.dataset)
      managerId =
        managerCard.dataset.managerId || managerCard.dataset.managerId;
    ...
    if (!managerId) {
      ...
      if (parentWithId)
        managerId =
          parentWithId.dataset.id || parentWithId.dataset.managerId || null;
    }

    // If the clicked button is a modify/edit button, call openEditModal
    if (
      target.classList.contains("modifica-dirigente") ||
      target.classList.contains("edit-manager")
    ) {
      if (!managerId) {
    ...
    // If the clicked button is a remove/delete button (supporting different class names), call remove
      ...
      target.classList.contains("remove-manager-btn") ||
      target.classList.contains("elimina-dirigente") ||
      target.classList.contains("remove-manager")
    ) {
      if (!managerId) {
  ...
  async searchUsers(query) {
    try {
      const response = await fetch(
        `/api/search-users?q=${encodeURIComponent(query)}&role=dirigente`,
        { credentials: "same-origin" }
      );
      const contentType = response.headers.get("content-type") || "";
      ...
      if (response.ok && data.users) {
        ...
        this.managerSuggestions.innerHTML =
          '<div class="p-3 text-muted">Nessun utente trovato</div>';
      }
    } catch (error) {
      console.error("Errore ricerca utenti:", error);
      this.managerSuggestions.innerHTML =
        '<div class="p-3 text-danger">Errore durante la ricerca</div>';
    }
  }
  ...
  async addManager() {
    if (!this.selectedUserId) {
      this.notificationManager.showError(
        "Seleziona un utente prima di aggiungere"
      );
      return;
    }
    ...
    try {
      const url = isUpdate
        ? `/squadre/${this.teamId}/dirigenti/${managerId}`
        : `/squadre/${this.teamId}/dirigenti`;
      ...
      if (response.ok && result.success) {
        const msg = isUpdate
          ? "Dirigente aggiornato con successo!"
          : "Dirigente aggiunto con successo!";
        this.notificationManager.showSuccess(msg);
        ...
        throw new Error(
          result.error ||
            `Errore durante ${
              isUpdate ? "l'aggiornamento" : "l'aggiunta"
            } del dirigente`
        );
      }
    } catch (error) {
  ...
  async removeManager(managerId) {
    ...
    showEntityDeleteModal({
      message:
        "Sei sicuro di voler rimuovere questo dirigente dalla squadra? Questa operazione è irreversibile.",
      ...
      onConfirm: async (id) => {
        ...
        try {
          const response = await fetch(
            `/squadre/${this.teamId}/dirigenti/${id}`,
            {
              method: "DELETE",
              credentials: "same-origin",
            }
          ...
          if (response.ok && result.success) {
            this.notificationManager.showSuccess(
              "Dirigente rimosso con successo!"
            );
            ...
            if (!el) {
              ...
              const btn =
                ...
                document.querySelector(`.elimina-dirigente[data-id="${id}"]`);
              if (btn)
                el = btn.closest(
                  ".col-xl-3, .col-lg-4, .col-md-6, .card, .player-card, .manager-card"
                );
            }
            if (el) el.remove();
            else
              console.warn(
                "Could not find DOM element for manager id",
                id,
            ...
            try {
              ...
              undoDiv.id = undoKey;
              undoDiv.className =
                "shadow-lg rounded p-2 bg-white border position-fixed";
              ...
              if (dismissBtn)
                dismissBtn.addEventListener("click", () => undoDiv.remove());
                ...
                undoBtn.addEventListener("click", async () => {
                  try {
                    ...
                    const resp = await fetch(
                      `/squadre/${this.teamId}/dirigenti/${id}/restore`,
                      {
                        method: "POST",
                        credentials: "same-origin",
                      }
                    ...
                    this.notificationManager.showError(
                      err.message || "Errore durante il ripristino"
                    );
                  } finally {
              ...
              setTimeout(() => {
                if (document.getElementById(undoKey))
                  document.getElementById(undoKey).remove();
              }, 8000);
            } catch (err) {
            ...
            throw new Error(
              result.error || "Errore durante la rimozione del dirigente"
            );
          }
        } catch (error) {
          console.error("Errore:", error);
          this.notificationManager.showError(
            error.message || "Errore durante la rimozione del dirigente"
          );
        } finally {
          this.loadingManager.hide();
        }
  ...
  async deleteManagerFromModal() {
    ...
    const ok = window.ShowModal
      ? await ShowModal.modalDelete(
          "Sei sicuro di voler eliminare questo dirigente?",
          "Conferma eliminazione"
        )
    ...
    try {
      ...
      if (response.ok && result.success) {
        this.notificationManager.showSuccess(
          "Dirigente eliminato con successo"
        );
      ...
      console.error(err);
      this.notificationManager.showError(
        err.message || "Errore durante l'eliminazione del dirigente"
      );
    } finally {
      this.loadingManager.hide();
    }
...
function showEntityDeleteModal(options) {
  ...
  body.textContent =
    options.message || "Sei sicuro di voler eliminare questo elemento?";
...
function createBackButton() {
  try {
    ...
    btn.id = "backButton";
    btn.className =
      "btn position-fixed top-0 start-0 m-3 d-flex align-items-center justify-content-center";
