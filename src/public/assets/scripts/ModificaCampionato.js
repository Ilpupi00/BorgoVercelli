...
async function loadTeams() {
  ...
  try {
    const response = await fetch(
      `/api/admin/campionati/${campionatoId}/squadre`
    );
    ...
    .map(
      (team) => `
        <tr>
            ...
            <td class="text-end">
                <button class="btn btn-sm btn-outline-danger" onclick="removeTeam('${escapeHtml(
                  team.nome
                )}')" title="Rimuovi">
...
async function addTeam() {
  ...
  try {
    ...
    window.AdminGlobal.ToastManager.show(
      error.message || "Errore aggiunta squadra",
      "error"
    );
  } finally {
...
async function removeTeam(squadraNome) {
  ...
  try {
    ...
    const response = await fetch(
      `/api/admin/campionati/${campionatoId}/squadre/${encodeURIComponent(
        squadraNome
      )}`,
      {
        method: "DELETE",
    ...
    console.error("Errore:", error);
    window.AdminGlobal.ToastManager.show(
      error.message || "Errore rimozione squadra",
      "error"
    );
  } finally {
    window.AdminGlobal.LoadingOverlay.hide();
