...
self.addEventListener("push", (event) => {
  ...
  const options = {
    ...
    renotify: true,
    requireInteraction:
      data.requireInteraction !== undefined ? data.requireInteraction : false,
    silent: false,
  };
  ...
  const forwardToClients = async () => {
    try {
      ...
      console.log(
        "[Service Worker] forwardToClients -> clients trovati:",
        allClients.length
      );
        ...
        await Promise.allSettled(
          allClients.map((client) =>
            client
              ...
              .catch((e) =>
                console.warn(
                  "[Service Worker] Impossibile postMessage a client:",
                  e
                )
              )
          )
      ...
      console.error(
        "[Service Worker] Errore forwarding message ai clients:",
        e
      );
      return 0;
    }
  ...
  const showNotificationSafely = async () => {
    try {
      ...
      console.log(
        "[Service Worker] showNotification -> options:",
      ...
      console.log(
        "[Service Worker] ✅ showNotification completato con successo"
      );
      return { shown: true };
    } catch (e) {
      ...
      try {
        console.log(
          "[Service Worker] Tentativo fallback con opzioni minime..."
        );
        ...
        console.error(
          "[Service Worker] ❌ Anche fallback showNotification fallita:",
          fallbackError
        );
        return {
          shown: false,
  ...
  event.waitUntil(
    (async () => {
      try {
        ...
        console.log(
          "[Service Worker]   - Notifica mostrata:",
          showResult.shown
        );
        ...
        console.error(
          "[Service Worker] ❌ Errore generale push handling:",
          error
        );
      }
    })()
  ...
  event.waitUntil(
    (async () => {
      try {
        ...
        console.log(
          "[Service Worker] Finestre aperte trovate:",
        ...
        for (const client of windowClients) {
          try {
            ...
            if (clientUrl.origin === targetUrl.origin) {
              console.log(
                "[Service Worker] Finestra esistente trovata, focus e navigazione"
              );
        ...
        if (clients.openWindow) {
          ...
          console.warn(
            "[Service Worker] openWindow non disponibile su questo browser"
          );
        }
      } catch (error) {
        console.error(
          "[Service Worker] ❌ Errore gestione click notifica:",
          error
        );
      }
    })()
  ...
  event.waitUntil(
    (async () => {
      try {
        ...
        console.error(
          "[Service Worker] ❌ Errore durante installazione:",
          error
        );
      }
    })()
  ...
  event.waitUntil(
    (async () => {
      try {
        ...
        console.log(
          "[Service Worker] 🎯 Service Worker ora attivo e in controllo"
        );
      } catch (error) {
...
self.addEventListener("unhandledrejection", (event) => {
  console.error(
    "[Service Worker] ❌ Promise rejection non gestita:",
    event.reason
  );
});
...
console.log(
  ...
  self.registration?.scope || "unknown"
);
