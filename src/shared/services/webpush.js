...
if (!vapidPublicKey || !vapidPrivateKey) {
  console.error(
    "[WEBPUSH] ATTENZIONE: Chiavi VAPID non configurate! Le notifiche push non funzioneranno."
  );
  console.error(
    "[WEBPUSH] Configura VAPID_PUBLIC_KEY e VAPID_PRIVATE_KEY nel file .env"
  );
} else {
...
async function loadSubscriptions() {
  try {
    ...
    console.error(
      "[WEBPUSH] Errore caricamento subscriptions dal database:",
      error
    );
    return [];
  }
...
async function addSubscription(
  ...
  try {
    ...
    if (isNew) {
      console.log(
        `[WEBPUSH] ✅ Subscription aggiunta per user ${normalizedUserId} (admin: ${isAdmin})`
      );
    } else {
      console.log(
        `[WEBPUSH] 🔄 Subscription aggiornata per user ${normalizedUserId} (admin: ${isAdmin})`
      );
    }
    ...
    console.error(
      "[WEBPUSH] ❌ Errore aggiunta/aggiornamento subscription:",
      error
    );
    throw error;
  }
...
async function removeSubscription(endpoint) {
  try {
    ...
    if (result.rows.length > 0) {
      console.log(
        `[WEBPUSH] 🗑️ Subscription rimossa: ${endpoint.substring(0, 50)}...`
      );
    } else {
      console.log(
        `[WEBPUSH] ⚠️ Subscription non trovata per rimozione: ${endpoint.substring(
          0,
          50
        )}...`
      );
...
async function sendNotificationToUsers(
  ...
  try {
    ...
    console.log(
      "[WEBPUSH] Subscriptions totali caricate:",
    ...
    const targetSubs = subscriptions.filter((s) =>
      userIdStrings.includes(String(s.userId))
    );
    console.log(
      "[WEBPUSH] Subscriptions trovate per questi user:",
      targetSubs.length
    );

    // Log subscriptions that have missing/undefined userId (helpful for debugging)
    const missingIdSubs = subscriptions.filter(
      (s) => s.userId === undefined || s.userId === null
    );
    if (missingIdSubs.length > 0) {
      console.warn(
        "[WEBPUSH] Attenzione - alcune subscription non hanno userId:",
        missingIdSubs.length
      );
    }
    ...
    if (targetSubs.length > 0) {
      ...
      targetSubs.forEach((sub, idx) => {
        console.log(
          `[WEBPUSH]   ${idx + 1}. userId: ${sub.userId}, admin: ${
            sub.isAdmin
    ...
    if (targetSubs.length === 0) {
      console.log(
        "[WEBPUSH] ⚠️ Nessuna subscription trovata per gli utenti specificati"
      );
      console.log(
        "[WEBPUSH] Suggerimento: Gli utenti devono accettare le notifiche nel browser"
      );
    ...
    const results = await Promise.allSettled(
      targetSubs.map((sub, idx) => {
        console.log(
          `[WEBPUSH] Invio ${idx + 1}/${targetSubs.length} a userId ${
            sub.userId
          }...`
        );
        ...
        return webpush
          ...
          .then(async () => {
            console.log(
              `[WEBPUSH] ✅ Notifica ${idx + 1} inviata con successo`
            );
          ...
          .catch(async (err) => {
            console.error(
              `[WEBPUSH] ❌ Errore invio notifica ${idx + 1}:`,
              err.message
            );
            ...
            if (err.statusCode === 410 || err.statusCode === 404) {
              console.log(
                `[WEBPUSH] 🗑️ Subscription scaduta/rimossa (${err.statusCode}) - rimozione`
              );
              await removeSubscription(sub.endpoint);
              return { removed: sub.endpoint, reason: "expired" };
            } else if (err.statusCode === 403) {
              console.error(
                `[WEBPUSH] 🚫 Errore 403 - possibile VAPID mismatch o subscription non valida`
              );
              console.error(
                `[WEBPUSH] Endpoint: ${sub.endpoint.substring(0, 80)}...`
              );
              await incrementErrorCount(sub.endpoint);
              throw new Error(
                `VAPID mismatch o subscription invalida: ${err.message}`
              );
            } else if (err.statusCode === 401) {
              console.error(
                `[WEBPUSH] 🔒 Errore 401 - autenticazione VAPID fallita`
              );
              throw new Error(`VAPID authentication failed: ${err.message}`);
            } else if (err.statusCode >= 500) {
              console.error(
                `[WEBPUSH] 💥 Errore server push service (${err.statusCode}) - retry possibile`
              );
              await incrementErrorCount(sub.endpoint);
              throw new Error(
                `Push service error ${err.statusCode}: ${err.message}`
              );
            } else {
              console.error(
                `[WEBPUSH] ⚠️ Errore generico (${err.statusCode || "unknown"})`
              );
              await incrementErrorCount(sub.endpoint);
              throw err;
            }
    ...
    results.forEach((r, idx) => {
      if (r.status === "rejected") {
        console.error(
          "[WEBPUSH] ❌ Rejected per userId",
          targetSubs[idx]?.userId
        );
        console.error("[WEBPUSH]    Motivo:", r.reason?.message || r.reason);
        if (r.reason?.stack)
          console.error("[WEBPUSH]    Stack:", r.reason.stack);
      } else if (r.value && r.value.removed) {
        console.warn(
          "[WEBPUSH] 🗑️ Subscription rimossa:",
          targetSubs[idx]?.endpoint?.substring(0, 50) + "..."
        );
      }
    });
...
async function sendNotificationToAdmins(payload, subscriptionsParam = null) {
  try {
    ...
    try {
      ...
      console.log(
        "[WEBPUSH] Admin subscriptions summary:",
        JSON.stringify(adminSummary, null, 2)
      );
    } catch (e) {
      console.warn(
        "[WEBPUSH] Impossibile serializzare adminSubs per summary",
        e && e.message
      );
    }
    ...
    if (adminSubs.length === 0) {
      ...
      console.log(
        "[WEBPUSH] Suggerimento: Gli admin devono accettare le notifiche nel browser"
      );
      return { sent: 0, failed: 0 };
    }
    ...
    const results = await Promise.allSettled(
      adminSubs.map((sub, idx) => {
        console.log(
          `[WEBPUSH] Invio admin ${idx + 1}/${adminSubs.length} a userId ${
            sub.userId
          }...`
        );
        ...
        return webpush
          ...
          .then(async () => {
            console.log(
              `[WEBPUSH] ✅ Notifica admin ${idx + 1} inviata con successo`
            );
            await updateSuccessTimestamp(sub.endpoint);
            return { success: true };
          })
          .catch(async (err) => {
            console.error(
              `[WEBPUSH] ❌ Errore invio notifica admin ${idx + 1}:`,
              err.message
            );
            console.error(`[WEBPUSH] Status Code:`, err.statusCode);
            if (err.body) console.error(`[WEBPUSH] Body:`, err.body);
            ...
            if (err.statusCode === 410 || err.statusCode === 404) {
              console.log(
                `[WEBPUSH] 🗑️ Subscription admin scaduta (${err.statusCode}) - rimozione`
              );
              await removeSubscription(sub.endpoint);
              return { removed: sub.endpoint, reason: "expired" };
            } else if (err.statusCode === 403) {
              console.error(
                `[WEBPUSH] 🚫 Admin subscription - errore 403 VAPID mismatch`
              );
              await incrementErrorCount(sub.endpoint);
              throw new Error(`VAPID mismatch (admin): ${err.message}`);
            } else if (err.statusCode === 401) {
              console.error(
                `[WEBPUSH] 🔒 Admin subscription - errore 401 auth VAPID`
              );
              throw new Error(
                `VAPID authentication failed (admin): ${err.message}`
              );
            } else if (err.statusCode >= 500) {
              console.error(
                `[WEBPUSH] 💥 Push service error ${err.statusCode} (admin) - retry possibile`
              );
              await incrementErrorCount(sub.endpoint);
              throw new Error(
                `Push service error ${err.statusCode} (admin): ${err.message}`
              );
            } else {
              await incrementErrorCount(sub.endpoint);
              throw err;
    ...
    results.forEach((r, idx) => {
      if (r.status === "rejected") {
        console.error(
          "[WEBPUSH] ❌ Rejected per admin userId",
          adminSubs[idx]?.userId
        );
        console.error("[WEBPUSH]    Motivo:", r.reason?.message || r.reason);
        if (r.reason?.stack)
          console.error("[WEBPUSH]    Stack:", r.reason.stack);
        ...
        console.warn(
          "[WEBPUSH] 🗑️ Admin subscription rimossa:",
          adminSubs[idx]?.endpoint?.substring(0, 50) + "..."
        );
      }
    });
...
async function sendNotificationToAll(payload, subscriptionsParam = null) {
  try {
    ...
    console.log(
      `[WEBPUSH] Invio broadcast a ${subscriptions.length} subscriptions...`
    );
    ...
    console.log(
      `[WEBPUSH] Notifiche broadcast - Inviate: ${sent}, Fallite: ${failed}, Rimosse: ${removed}`
    );
    return { sent, failed, removed, results };
  } catch (error) {
...
async function cleanupFailedSubscriptions(maxErrors = 5) {
  try {
    ...
    console.log(
      `[WEBPUSH] 🧹 Pulizia completata: ${result.rows.length} subscriptions rimosse`
    );
