...
router.get("/admin/verify-smtp", async (req, res) => {
  ...
  if (!expected)
    return res
      .status(400)
      .json({ error: "ADMIN_VERIFY_TOKEN not configured on server" });
...
router.get("/admin/send-test-resend", async (req, res) => {
  ...
  if (!process.env.RESEND_API_KEY) {
    return res
      .status(400)
      .json({ error: "RESEND_API_KEY not configured on server" });
  }

  const to = req.query.to;
  if (!to)
    return res.status(400).json({ error: "Missing required query param: to" });
...
router.get("/campionato", async (req, res) => {
  try {
    ...
    if (campionati.length > 0) {
      ...
      classifica =
        (await daoCampionati.getClassificaByCampionatoId(
          selectedCampionato.id
        )) || [];
      regole = {
    ...
    res.render("campionato", {
      ...
      classifica,
      isLoggedIn,
      selectedCampionato: selectedCampionato ? selectedCampionato.id : null,
      regole,
    });
  } catch (error) {
  ...
  try {
    ...
    const dirigentiSquadre = await new Promise((resolve, reject) => {
      ...
      db.all(sql, [], (err, rows) => {
        if (err) {
          console.error(
            "[route /societa] Errore query dirigenti squadre:",
            err
          );
          return resolve([]);
        }
    ...
    if (process.env.NODE_ENV === "development") {
      console.log(
        "[route /societa] Dirigenti raw rows:",
        JSON.stringify(dirigentiSquadre, null, 2)
      );
    }
    ...
    if (process.env.NODE_ENV === "development") {
      console.log(
        "[route /societa] Totale membri:",
        tuttiMembriSafe.length,
        "(società:",
        membriSocieta.length,
        ", dirigenti:",
        dirigentiSquadre.length,
        ")"
      );
    }
...
router.get("/prenotazione", async (req, res) => {
  try {
    ...
    for (const campo of campi) {
      try {
        const orari = await daoPrenotazione.getDisponibilitaCampo(
          campo.id,
          oggi
        );
...
router.get("/scrivi/recensione", (req, res) => {
  ...
  try {
    ...
    console.error(
      "Errore nel caricamento della pagina scrivi recensione:",
      error
    );
    res.render("scrivi_recensione", {
      isLogged: req.isAuthenticated(),
...
router.get("/recensioni/all", async (req, res) => {
  try {
    ...
    const averageRating =
      totalReviews > 0
        ? recensioni.reduce((sum, r) => sum + r.valutazione, 0) / totalReviews
        : 0;
    const ratingCounts = {};
    for (let i = 1; i <= 5; i++) {
...
router.get(
  ...
  (req, res) => {
    try {
      ...
      console.error(
        "Errore nel rendering della pagina di creazione notizia:",
        error
      );
      res.status(500).send("Internal Server Error");
    }
...
router.post("/contatti", async (req, res) => {
  try {
    ...
    console.log("[POST /contatti] payload:", {
      name,
      email,
      subject,
      phone,
    ...
    if (process.env.EMAIL_DEBUG)
      console.log("[POST /contatti] sendEmail result:", info);
