...
router.post("/push/subscribe", express.json(), async (req, res) => {
  try {
    ...
    console.log(
      `[PUSH] ✅ Salvataggio subscription per user ${userId} (admin: ${isAdmin})`
    );
...
router.post("/push/subscribe-anon", express.json(), async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ success: true, message: "Subscription anon salvata" });
  } catch (error) {
...
router.post("/push/unsubscribe", express.json(), async (req, res) => {
  try {
    ...
    if (isAuthenticated) {
      const ownerId =
        sub.user_id !== null && sub.user_id !== undefined
          ? String(sub.user_id)
          : null;
      if (ownerId !== null && String(req.user.id) !== ownerId && !isAdmin) {
        return res
          .status(403)
          .json({ error: "Non autorizzato a rimuovere questa subscription" });
      }
    } else {
    ...
    if (isAuthenticated && sub.user_id && sub.user_id !== 0) {
      try {
        ...
        const username =
          req.user.username || req.user.email || `Utente ${req.user.id}`;
        ...
        console.log(
          `[PUSH] Notifica disattivazione inviata agli admin per utente ${sub.user_id}`
        );
      } catch (notifErr) {
        console.error(
          "[PUSH] Errore invio notifica disattivazione agli admin:",
          notifErr
        );
...
router.post("/push/subscribe-error", express.json(), (req, res) => {
  try {
    const body = req.body || {};
    console.error(
      "[PUSH] Client subscribe error report:",
...
router.get("/push/my-subscriptions", async (req, res) => {
  try {
    ...
    console.log(
      `[PUSH] /push/my-subscriptions -> userId=${myId}, found=${mySubs.length}`
    );
  ...
  try {
    ...
    const mySubs = all.filter(
      (s) =>
        ...
        (s.userId === 0 && String(req.user.id) === "0")
    );
...
router.post("/push/debug-send-myself", express.json(), async (req, res) => {
  try {
    ...
    const {
      ...
      body = "Questa è una notifica di test inviata a te",
      url = "/",
    } = req.body || {};
...
router.post("/push/test", express.json(), async (req, res) => {
  try {
    ...
    if (to === "me") {
      const userId = req.user.id;
      const result = await pushService.sendNotificationToUsers(
        [userId],
        payload
      );
