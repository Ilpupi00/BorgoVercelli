  ...
  if (req.isAuthenticated && req.isAuthenticated()) {
    ...
    if (!req.user) {
      ...
      return res
        .status(401)
        .render("error", {
          message: "Accesso negato: devi essere loggato",
          error: { status: 401 },
        });
    }
    ...
    if (req.user.isBannato && req.user.isBannato()) {
      ...
      if (
        ...
        return res.status(403).json({
          ...
          message:
            "Il tuo account è stato bannato. Contatta l'amministrazione per maggiori informazioni.",
        });
      } else {
        return res.status(403).render("error", {
          message:
            "Account bannato: il tuo account è stato bannato dal sito. Contatta l'amministrazione per maggiori informazioni.",
          error: { status: 403 },
        });
      }
    }
...
const isAdmin = function (req, res, next) {
  ...
  if (req.headers.accept && req.headers.accept.includes("application/json")) {
    ...
    res
      .status(403)
      .render("error", {
        message: "Accesso negato: devi essere amministratore",
        error: { status: 403 },
      });
  }
};
...
const isDirigente = function (req, res, next) {
  ...
  if (req.headers.accept && req.headers.accept.includes("application/json")) {
    ...
    res
      .status(403)
      .render("error", {
        message: "Accesso negato: devi essere dirigente",
        error: { status: 403 },
      });
  }
};
...
const isAdminOrDirigente = function (req, res, next) {
  ...
  if (req.headers.accept && req.headers.accept.includes("application/json")) {
    ...
    res
      .status(403)
      .render("error", {
        message: "Accesso negato: devi essere amministratore o dirigente",
        error: { status: 403 },
      });
  }
};
...
const isSquadraDirigente = async function (req, res, next) {
  ...
  try {
    ...
    const gestisceSquadra =
      dirigenti && dirigenti.some((d) => d.squadra_id == req.params.id);
    ...
    if (gestisceSquadra) {
      ...
      if (
        ...
        res
          .status(403)
          .render("error", {
            message: "Accesso negato: devi essere dirigente della squadra",
            error: { status: 403 },
          });
      }
    }
...
const canEditNotizia = async function (req, res, next) {
  ...
  if (
    ...
    user.tipo_utente_id === 2 ||
    user.tipo_utente_id === 3 ||
    user.tipo_utente_id === 5
  ) {
    return next();
  }
  ...
  if (req.headers.accept && req.headers.accept.includes("application/json")) {
    ...
    res
      ...
      .render("error", {
        message:
          "Accesso negato: non hai permessi per modificare questa notizia",
        error: { status: 403 },
      });
  }
};
...
const isGestoreCampo = function (req, res, next) {
  ...
  if (req.headers.accept && req.headers.accept.includes("application/json")) {
    ...
    res
      .status(403)
      .render("error", {
        message: "Accesso negato: devi essere gestore campo",
        error: { status: 403 },
      });
  }
};
...
const canManageCampi = function (req, res, next) {
  ...
  if (req.headers.accept && req.headers.accept.includes("application/json")) {
    ...
    res
      .status(403)
      .render("error", {
        message: "Accesso negato: non hai permessi per gestire i campi",
        error: { status: 403 },
      });
  }
};
