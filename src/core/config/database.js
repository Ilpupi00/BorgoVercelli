...
if (!process.env.DATABASE_URL) {
  ...
  console.error(
    "[database] L'applicazione è configurata per connettersi solo a Postgres tramite DATABASE_URL (es. fornita da Railway)."
  );
  console.error(
    "[database] Assicurati di configurare DATABASE_URL nelle variabili d'ambiente."
  );
  ...
  throw new Error(
    "DATABASE_URL non impostata: configura la variabile d'ambiente DATABASE_URL"
  );
}
...
const db = {
  get: (sql, params, cb) => {
    ...
    pool
      ...
      .then(
        (res) =>
          cb && cb(null, res.rows && res.rows[0] ? res.rows[0] : undefined)
      )
      .catch((err) => cb && cb(err));
  },
  all: (sql, params, cb) => {
