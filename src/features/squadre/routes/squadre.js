...
router.post("/createsquadra", isLoggedIn, isAdmin, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: err.error || "Errore durante la creazione della squadra",
      });
  }
});
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      await daoSquadre.updateSquadra(
        id,
        nome,
        parseInt(annoFondazione),
        id_immagine
      ...
      res
        .status(500)
        .json({
          error: err.error || "Errore durante l'aggiornamento della squadra",
        });
    }
  }
);
...
router.delete("/deletesquadra/:id", isLoggedIn, isAdmin, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: err.error || "Errore durante l'eliminazione della squadra",
      });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      const giocatore = await daoSquadre.createGiocatore({
        ...
        cognome,
        ruolo,
        numero_maglia: parseInt(numero_maglia),
        data_nascita,
        nazionalita,
        squadra_id: parseInt(squadra_id),
        id_immagine: immagineId,
      });
      ...
      res
        .status(500)
        .json({ error: "Errore durante la creazione del giocatore" });
    }
  }
);
...
router.put(
  ...
  async (req, res) => {
    try {
      const { id } = req.params;
      const { nome, cognome, ruolo, numero_maglia, data_nascita, nazionalita } =
        req.body;
      let immagineId = null;
      if (req.file) {
        immagineId = await daoGalleria.uploadImmagine(req.file, "giocatore");
      ...
      res
        .status(500)
        .json({ error: "Errore durante l'aggiornamento del giocatore" });
    }
  }
);
...
router.post("/createdirigente", isLoggedIn, isAdmin, async (req, res) => {
  try {
    const { utente_id, ruolo, data_nomina, data_scadenza, squadra_id } =
      req.body;
    ...
    res
      .status(500)
      .json({ error: "Errore durante la creazione del dirigente" });
  }
});
...
router.put("/updatedirigente/:id", isLoggedIn, isAdmin, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Errore durante l'aggiornamento del dirigente" });
  }
});
...
router.delete("/deletedirigente/:id", isLoggedIn, isAdmin, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Errore durante l'eliminazione del dirigente" });
  }
});
...
router.get("/getsquadra/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: err.error || "Errore durante il recupero della squadra" });
  }
});
...
router.get("/squadre/gestione/:id", isAdminOrDirigente, async (req, res) => {
  try {
    ...
    if (req.user.tipo_utente_id === 2) {
      ...
      const gestisce =
        Array.isArray(dirigenti) &&
        dirigenti.some((d) => String(d.squadra_id) === String(req.params.id));
      if (!gestisce) {
    ...
    try {
      squadra.dirigenti = await daoDirigenti.getDirigentiBySquadra(
        req.params.id
      );
    } catch (err) {
    ...
    for (let giocatore of squadra.giocatori) {
      if (giocatore.id_immagine) {
        giocatore.immagine = await daoGalleria.getImmagineById(
          giocatore.id_immagine
        );
      }
    }
    ...
    for (let dirigente of squadra.dirigenti) {
      ...
      if (typeof dirigente.immagine === "string") {
        ...
        dirigente.immagine &&
        typeof dirigente.immagine === "object" &&
        dirigente.immagine.url
      ) {
        // già nella forma corretta
      } else if (
        dirigente.immagine_id &&
        (typeof dirigente.immagine_id === "string" ||
          typeof dirigente.immagine_id === "number")
      ) {
    ...
    console.error(
      "Errore nel caricamento della pagina modifica squadra:",
      error
    );
...
router.post("/squadre/:id/dirigenti", isSquadraDirigente, async (req, res) => {
  try {
    ...
    console.debug(
      "[POST /squadre/:id/dirigenti] user=",
    ...
    try {
      ...
      console.warn(
        "[POST /squadre/:id/dirigenti] utente non trovato:",
        userId,
        e && e.error ? e.error : e
      );
    ...
    try {
      ...
      try {
        ...
        sqlite.get(sqlGetDir, [addResult.id], (err2, dirigenteRow) => {
          if (err2) {
            console.error(
              "[POST /squadre/:id/dirigenti] Errore recupero dirigente creato:",
              err2
            );
            return res.json({
              success: true,
              message: "Dirigente aggiunto con successo",
            });
          }
          console.log(
            "[POST /squadre/:id/dirigenti] Dirigente DB row:",
            JSON.stringify(dirigenteRow, null, 2)
          );
          // Assicuriamoci di includere lo stato 'attivo' nella risposta
          const attivo =
            dirigenteRow && typeof dirigenteRow.attivo !== "undefined"
              ? dirigenteRow.attivo
              : null;
          return res.json({
            success: true,
            message: "Dirigente aggiunto con successo",
            dirigente: dirigenteRow,
            attivo,
          });
        });
      } catch (e) {
        ...
        console.error(
          "[POST /squadre/:id/dirigenti] Errore imprevisto recupero dirigente dopo add:",
          e
        );
        ...
        sqlite.get(
          fallbackSql,
          [userId, squadra_id, ruolo || "Dirigente"],
          (fbErr, fbRow) => {
            if (fbErr) {
              console.error(
                "[POST /squadre/:id/dirigenti] Fallback query error:",
                fbErr
              );
              return res.json({
                success: true,
            ...
            console.log(
              "[POST /squadre/:id/dirigenti] Fallback dirigente row:",
              JSON.stringify(fbRow, null, 2)
            );
            const attivo =
              fbRow && typeof fbRow.attivo !== "undefined"
                ? fbRow.attivo
                : null;
            return res.json({
              success: true,
      ...
      console.error(
        "[POST /squadre/:id/dirigenti] Errore DB aggiunta dirigente:",
        err
      );
      return res
        .status(500)
        .json({
          error:
            err && err.error
              ? err.error
              : "Errore durante l'aggiunta del dirigente",
        });
    }
  } catch (err) {
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          error: err.error || "Errore durante l'aggiornamento del dirigente",
        });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          error: err.error || "Errore durante la rimozione del dirigente",
        });
    }
  }
);
...
router.post(
  "/squadre/:id/dirigenti/:managerId/restore",
  isSquadraDirigente,
  async (req, res) => {
    try {
      const { managerId } = req.params;
      ...
      res.json({
        success: true,
        message: "Dirigente ripristinato con successo",
      });
    } catch (err) {
      ...
      res
        .status(500)
        .json({
          error: err.error || "Errore durante il ripristino del dirigente",
        });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      res
        ...
        .json({
          error:
            err.error || "Errore durante il ripristino massivo dei dirigenti",
        });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          error: err.error || "Errore durante l'aggiunta del giocatore",
        });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          error: err.error || "Errore durante l'aggiornamento del giocatore",
        });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          error: err.error || "Errore durante la rimozione del giocatore",
        });
    }
  }
);
...
router.get("/modifica_squadra", isLoggedIn, isAdmin, async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(
        "Giocatori caricati per squadra",
        id,
        ":",
        squadra.giocatori ? squadra.giocatori.length : "undefined"
      );
    } catch (err) {
      console.error("Errore caricamento giocatori:", err);
      squadra.giocatori = [];
    ...
    try {
      ...
      console.log(
        "[GET /modifica_squadra] Dirigenti (inclusi inattivi) caricati per squadra",
        id,
        ":",
        squadra.dirigenti ? squadra.dirigenti.length : "undefined"
      );
      if (squadra.dirigenti && squadra.dirigenti.length > 0) {
        console.log(
          "[GET /modifica_squadra] Primo dirigente:",
          JSON.stringify(squadra.dirigenti[0], null, 2)
        );
      }
    } catch (err) {
      console.error("Errore caricamento dirigenti:", err);
  ...
  try {
    ...
    if (!Array.isArray(dirigenti) || dirigenti.length === 0) {
      return res.render("error", {
        message: "Non sei un dirigente di nessuna squadra",
        error: { status: 403 },
      });
    }
