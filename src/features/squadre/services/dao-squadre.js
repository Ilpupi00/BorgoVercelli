...
exports.getSquadre = async () => {
  ...
  return new Promise((resolve, reject) => {
    db.all(sql, async (err, squadre) => {
      ...
      const squadreConDirigenti = await Promise.all(
        squadre.map(async (squadra) => {
          const dirigenti = await daoDirigenti.getDirigentiBySquadra(
            squadra.id
          );
...
exports.createSquadra = function (nome, annoFondazione) {
  const sql =
    "INSERT INTO SQUADRE (nome, anno_fondazione) VALUES (?, ?) RETURNING id";
  return new Promise((resolve, reject) => {
  ...
  if (id_immagine !== null) {
    ...
    const oldImage = await new Promise((resolve, reject) => {
      db.get(selectSql, [parseInt(id)], (err, row) => {
        if (err) {
          console.error(
            "[updateSquadra] Errore recupero vecchia immagine:",
            err
          );
  ...
  return new Promise((resolve, reject) => {
    db.run(sql, [parseInt(id)], function (err, result) {
      if (err) {
        ...
        return reject({
          error: "Errore nella cancellazione della squadra: " + err.message,
        });
      }
      const changes =
        result && typeof result.rowCount === "number" ? result.rowCount : 0;
...
exports.getSquadraById = function (id) {
  ...
  return new Promise((resolve, reject) => {
    db.get(sql, [parseInt(id)], async (err, squadra) => {
      if (err) {
        return reject({
          error: "Errore nel recupero della squadra: " + err.message,
        });
      }
      if (!squadra) {
...
exports.getGiocatoriBySquadra = function (squadraId) {
  ...
  return new Promise((resolve, reject) => {
    db.all(sql, [parseInt(squadraId)], (err, rows) => {
      if (err) {
        return reject({
          error: "Errore nel recupero dei giocatori: " + err.message,
        });
      }
      const giocatori = (rows || []).map(makeGiocatore);
      resolve(giocatori);
    ...
    db.run(
      ...
      function (err, result) {
        if (err) {
          ...
          return reject({
            error: "Errore nella creazione del giocatore: " + err.message,
          });
        }
        resolve({
          id: result.rows[0].id,
...
exports.deleteGiocatore = function (id) {
  const sql =
    "UPDATE GIOCATORI SET attivo = false, data_fine_tesseramento = NOW() WHERE id = ?";
  return new Promise((resolve, reject) => {
    db.run(sql, [parseInt(id)], function (err, result) {
      if (err) {
        console.error("Errore SQL delete giocatore:", err);
        return reject({
          error: "Errore nella rimozione del giocatore: " + err.message,
        });
      }
      const changes =
        result && typeof result.rowCount === "number" ? result.rowCount : 0;
  ...
  return new Promise((resolve, reject) => {
    db.run(
      ...
      function (err, result) {
        if (err) {
          return reject({
            error: "Errore nell'aggiunta del giocatore: " + err.message,
          });
        }
        // Recupera il giocatore appena creato
        exports
          .getGiocatoreById(result.rows[0].id)
...
exports.removeGiocatore = function (id) {
  const sql =
    "UPDATE GIOCATORI SET attivo = false, updated_at = NOW() WHERE id = ?";
  return new Promise((resolve, reject) => {
...
exports.addDirigente = function (squadraId, email) {
  ...
  const sqlInsertDirigente =
    "INSERT INTO DIRIGENTI_SQUADRE (squadra_id, utente_id) VALUES (?, ?)";

  return new Promise((resolve, reject) => {
    db.get(sqlGetUser, [email], (err, user) => {
      if (err) {
        return reject({
          error: "Errore nel recupero dell'utente: " + err.message,
        });
      }
      if (!user) {
      ...
      const sqlCheck =
        "SELECT id FROM DIRIGENTI_SQUADRE WHERE squadra_id = ? AND utente_id = ?";
      db.get(sqlCheck, [parseInt(squadraId), user.id], (err, existing) => {
        ...
        if (existing) {
          return reject({
            error: "Questo utente è già dirigente di questa squadra",
          });
        }
        ...
        db.run(
          ...
          [parseInt(squadraId), user.id],
          function (err, result) {
            if (err) {
              return reject({
                error: "Errore nell'aggiunta del dirigente: " + err.message,
              });
            }
...
exports.removeDirigente = function (squadraId, dirigenteId) {
  ...
  return new Promise((resolve, reject) => {
    db.run(
      sql,
      [parseInt(squadraId), parseInt(dirigenteId)],
      function (err, result) {
        if (err) {
          return reject({
            error: "Errore nella rimozione del dirigente: " + err.message,
          });
        }
        const changes =
          result && typeof result.rowCount === "number" ? result.rowCount : 0;
  ...
  return new Promise((resolve, reject) => {
    db.run(
      ...
      function (err, result) {
        if (err) {
          return reject({
            error: "Errore aggiornamento giocatore: " + err.message,
          });
        }
        const changes =
          result && typeof result.rowCount === "number" ? result.rowCount : 0;
