...
exports.getDirigentiBySquadra = function (squadraId) {
  return new Promise((resolve, reject) => {
    console.log(
      "[dao-dirigenti] getDirigentiBySquadra chiamata con squadraId:",
      squadraId
    );
    ...
    db.all(debugSql, [], (debugErr, debugRows) => {
      if (!debugErr) {
        console.log(
          "[dao-dirigenti] DEBUG - Tutti i dirigenti in tabella:",
    ...
    db.all(sql, [squadraId], (err, rows) => {
      if (err) {
        console.error(
          "[dao-dirigenti] Errore query getDirigentiBySquadra:",
          err
        );
        return reject({ error: "Error retrieving dirigenti: " + err.message });
      }
      console.log(
        "[dao-dirigenti] Dirigenti trovati per squadra",
        squadraId,
        ":",
        rows ? rows.length : 0
      );
...
exports.getDirigentiBySquadraAll = function (squadraId) {
  return new Promise((resolve, reject) => {
    console.log(
      "[dao-dirigenti] getDirigentiBySquadraAll chiamata con squadraId:",
      squadraId
    );
    const sql = `
      SELECT ds.*, u.nome, u.cognome, u.email, i.url as immagine
    ...
    db.all(sql, [squadraId], (err, rows) => {
      if (err) {
        console.error(
          "[dao-dirigenti] Errore query getDirigentiBySquadraAll:",
          err
        );
        return reject({ error: "Error retrieving dirigenti: " + err.message });
      }
      console.log(
        "[dao-dirigenti] (ALL) Dirigenti trovati per squadra",
        squadraId,
        ":",
        rows ? rows.length : 0
      );
...
exports.getDirigentiSocietari = function () {
  return new Promise((resolve, reject) => {
    ...
    db.all(sql, [], (err, rows) => {
      if (err)
        return reject({
          error: "Error retrieving dirigenti societari: " + err.message,
        });
      resolve((rows || []).map(makeDirigenteSquadra));
    });
  });
...
exports.addDirigente = function (dirigente) {
  return new Promise((resolve, reject) => {
    const utente_id = dirigente.utente_id
      ? parseInt(dirigente.utente_id, 10)
      : null;
    const squadra_id = dirigente.squadra_id
      ? parseInt(dirigente.squadra_id, 10)
      : null;
    const ruolo = dirigente.ruolo || "Dirigente";
    const now = moment().format("YYYY-MM-DD HH:mm:ss");

    if (!utente_id || !Number.isInteger(utente_id))
      return reject({ error: "utente_id non valido" });
    ...
    db.get(
      checkSql,
      [utente_id, squadra_id || null, ruolo],
      (checkErr, existing) => {
        if (checkErr) {
          return reject({
            error: "Errore controllo dirigente esistente: " + checkErr.message,
          });
        }
        ...
        if (existing && existing.attivo) {
          return reject({
            error:
              "Questo utente è già dirigente attivo per questa squadra con lo stesso ruolo",
          });
        }
          ...
          return db.run(
            ...
            function (reactivateErr, reactivateResult) {
              if (reactivateErr) {
                return reject({
                  error:
                    "Errore riattivazione dirigente: " + reactivateErr.message,
                });
              }
              console.log("[dao-dirigenti] Dirigente riattivato:", existing.id);
              resolve({
                id: existing.id,
                message: "Dirigente riattivato con successo",
              });
            }
          );
        }
          ...
          function (insertErr, insertResult) {
            if (insertErr) {
              ...
              if (
                ...
                msg.includes("UNIQUE") ||
                (insertErr.code && insertErr.code === "23505")
              ) {
                return reject({
                  error:
                    "Questo utente è già dirigente per questa squadra con lo stesso ruolo",
                });
              }
            ...
            resolve({
              id: insertedId,
              message: "Dirigente added successfully",
            });
          }
        );
      }
...
exports.removeDirigente = function (id) {
  return new Promise((resolve, reject) => {
    ...
    db.run(sql, [id], function (err, result) {
      ...
      const changes =
        result && typeof result.rowCount === "number"
          ? result.rowCount
...
exports.getDirigenteByUserId = function (userId) {
  return new Promise((resolve, reject) => {
    ...
    db.all(sql, [userId], (err, dirigenti) => {
      if (err)
        return reject({
          error: "Errore nel recupero del dirigente: " + err.message,
        });
      resolve(dirigenti || []);
    });
  });
  ...
  return new Promise((resolve, reject) => {
    db.run(
      ...
      function (err, result) {
        if (err)
          return reject({
            error: "Errore creazione dirigente: " + err.message,
          });
...
exports.updateDirigente = async (id, dirigenteData) => {
  ...
  return new Promise((resolve, reject) => {
    db.run(sql, params, function (err, result) {
      if (err)
        return reject({
          error: "Errore aggiornamento dirigente: " + err.message,
        });
      const changes =
        result && typeof result.rowCount === "number"
          ? result.rowCount
...
exports.restoreAllDirigenti = function () {
  return new Promise((resolve, reject) => {
    ...
    db.run(sql, [], function (err, result) {
      if (err)
        return reject({
          error: "Errore ripristino massivo dirigenti: " + err.message,
        });
      const changes =
        result && typeof result.rowCount === "number"
          ? result.rowCount
