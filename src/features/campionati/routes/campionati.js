...
router.get("/", async (req, res) => {
  try {
    ...
    if (req.query.id) {
      ...
      if (!isNaN(campionatoId)) {
        ...
        if (campionato) {
          ...
          classifica = await daoCampionati.getClassificaByCampionatoId(
            campionatoId
          );
    ...
    if (!selectedCampionato && campionati.length > 0) {
      ...
      const primoCampionato = await daoCampionati.getCampionatoById(
        campionati[0].id
      );
      if (primoCampionato) {
        regole = {
...
router.get("/classifica/:id", async (req, res) => {
  try {
    ...
    const classifica = await daoCampionati.getClassificaByCampionatoId(
      campionatoId
    );
    // Recupera regole per la legenda dinamica
    const regoleCampionato = await daoCampionati.getCampionatoById(
      campionatoId
    );
