...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const insertResult = await daoGalleria.insertImmagine(
        filePath,
        now,
        now,
        descrizione
...
router.put("/UpdateImmagine/:id", isLoggedIn, isAdmin, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: err.error || "Errore durante l'aggiornamento dell'immagine",
      });
  }
});
...
router.delete("/DeleteImmagine/:id", isLoggedIn, isAdmin, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: err.error || "Errore durante l'eliminazione dell'immagine",
      });
  }
});
