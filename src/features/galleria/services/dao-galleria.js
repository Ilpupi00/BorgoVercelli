  ...
  return new Promise((resolve, reject) => {
    db.all(sql, (err, immagini) => {
      if (err) {
        ...
        return reject({
          error: "Errore nel recupero delle immagini: " + err.message,
        });
      }
  ...
  return new Promise((resolve, reject) => {
    db.get(sql, [parseInt(id)], (err, row) => {
      if (err) {
        ...
        return reject({
          error: "Errore nel recupero dell'immagine: " + err.message,
        });
      }
 ...
 */
exports.insertImmagine = function (
  url,
  created_at,
  updated_at,
  descrizione = ""
  ...
  const sql =
    "INSERT INTO IMMAGINI (url, tipo, descrizione, created_at, updated_at) VALUES (?, ?, ?, ?, ?) RETURNING id;";
  return new Promise((resolve, reject) => {
    db.run(
      sql,
      [url, "upload della Galleria", descrizione, created_at, updated_at],
      function (err, result) {
        if (err) {
          console.error("Errore SQL insert:", err);
          return reject({
            error: "Errore nell'inserimento dell'immagine: " + err.message,
          });
        }
...
exports.updateImmagine = function (id, descrizione) {
  const sql =
    "UPDATE IMMAGINI SET descrizione = ?, updated_at = ? WHERE id = ?;";
  return new Promise((resolve, reject) => {
    ...
    db.run(sql, [descrizione, now, parseInt(id)], function (err, result) {
      if (err) {
        ...
        return reject({
          error: "Errore nell'aggiornamento dell'immagine: " + err.message,
        });
      }
      const changes =
    ...
    db.get(selectSql, [parseInt(id)], (err, row) => {
      ...
      db.run(deleteSql, [parseInt(id)], function (err, result) {
        if (err) {
          ...
          return reject({
            error: "Errore nella cancellazione dell'immagine: " + err.message,
          });
        }
        const changes =
          result && typeof result.rowCount === "number" ? result.rowCount : 0;
...
exports.insertImmagineNotizia = function (url, entita_id, ordine) {
  const sql =
    "INSERT INTO IMMAGINI (url, tipo, entita_riferimento, entita_id, ordine, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id";
  return new Promise((resolve, reject) => {
    const now = new Date().toISOString();
    db.run(
      sql,
      [url, "notizia", "notizia", entita_id, ordine, now, now],
      function (err, result) {
        if (err) {
          console.error("Errore SQL insert immagine notizia:", err);
          return reject({
            error:
              "Errore nell'inserimento dell'immagine notizia: " + err.message,
          });
        }
