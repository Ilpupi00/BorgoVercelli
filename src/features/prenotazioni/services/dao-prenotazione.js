          ...
          return new Promise((resImg, rejImg) => {
            db.all(
              ...
              (errImg, imgRows) => {
                ...
                campo.immagini = Array.isArray(imgRows)
...
exports.getDisponibilitaCampo = async (campoId, data) => {
  ...
  let orariDisponibili = orariCampo.map((o) => ({
    inizio: o.ora_inizio,
    fine: o.ora_fine,
  }));
  ...
  return new Promise((resolve, reject) => {
    ...
    db.all(prenSql, [campoId, dataNorm], (err, rows) => {
      ...
      const orariOccupati = prenotazioni.map((p) => ({
        inizio: p.ora_inizio,
        fine: p.ora_fine,
      }));
      function toMin(ora) {
...
exports.prenotaCampo = async ({
  ...
  return new Promise((resolve, reject) => {
    db.get(
      `SELECT * FROM PRENOTAZIONI WHERE campo_id = ? AND data_prenotazione = ? AND ora_inizio = ? AND ora_fine = ? AND stato != 'annullata' AND stato != 'rifiutata'`,
      [campo_id, dataNorm, ora_inizio, ora_fine],
      (err, row) => {
        if (err) return reject(err);
        if (row)
          return resolve({ error: "Orario già prenotato (duplicato esatto)" });
        // Nuove prenotazioni iniziano con stato 'in_attesa' e devono essere accettate dall'admin
        db.run(
          `INSERT INTO PRENOTAZIONI (campo_id, utente_id, squadra_id, data_prenotazione, ora_inizio, ora_fine, tipo_attivita, note, telefono, codice_fiscale, tipo_documento, numero_documento, stato, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'in_attesa', NOW(), NOW()) RETURNING id`,
          [
            campo_id,
            utente_id || null,
            squadra_id || null,
          ...
          function (err, result) {
            if (err) {
              ...
              if (
                err.code === "23P01" ||
                (err.message && err.message.includes("prenotazioni_no_overlap"))
              ) {
                return resolve({
...
exports.getAllPrenotazioni = async () => {
  return new Promise((resolve, reject) => {
    ...
    db.all(sql, [], (err, prenotazioni) => {
      if (err) {
        return reject({
          error: "Error retrieving prenotazioni: " + err.message,
        });
      }
...
exports.getPrenotazioneById = async (id) => {
  return new Promise((resolve, reject) => {
    ...
    db.get(sql, [id], (err, row) => {
      if (err) {
        return reject({
          error: "Error retrieving prenotazione: " + err.message,
        });
      }
      resolve(row);
    });
 ...
 * @throws {Error} In caso di errore DB
 */
exports.updatePrenotazione = async (
  id,
  {
    campo_id,
    utente_id,
  ...
  return new Promise((resolve, reject) => {
    db.run(
      `UPDATE PRENOTAZIONI SET campo_id = ?, utente_id = ?, squadra_id = ?, data_prenotazione = ?, ora_inizio = ?, ora_fine = ?, tipo_attivita = ?, note = ?, telefono = ?, codice_fiscale = ?, tipo_documento = ?, numero_documento = ?, updated_at = NOW() WHERE id = ?`,
      [
        campo_id,
        utente_id || null,
        squadra_id || null,
  ...
  return new Promise((resolve, reject) => {
    ...
    db.get(
      ...
      (errBefore, rowBefore) => {
        ...
        db.run(
          `DELETE FROM PRENOTAZIONI WHERE stato = 'scaduta'`,
          function (err, result) {
            if (err) {
              console.error(
...
exports.getPrenotazioniByUserId = async (userId) => {
  return new Promise((resolve, reject) => {
    ...
    db.all(sql, [userId], (err, prenotazioni) => {
      if (err) {
        return reject({
          error: "Error retrieving user prenotazioni: " + err.message,
        });
      }
      resolve(prenotazioni || []);
    });
