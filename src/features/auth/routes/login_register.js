...
router.post("/registrazione", validateRegistrazione, async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({
        error: err.error || err.message || "Errore durante la registrazione",
      });
  }
});
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!req.file) {
      ...
      return res
        .status(400)
        .json({ success: false, error: "File non valido o danneggiato." });
    }
    ...
    if (req.file.size > maxSize) {
      return res
        .status(400)
        .json({ success: false, error: "File troppo grande. Massimo 5MB." });
    }
...
router.post("/api/user/change-password", validateChangePassword, async (req, res) => {
  ...
  try {
    ...
    res
      .status(400)
      .json({
        error: err.error || err.message || "Errore durante il cambio password",
      });
  }
});
...
router.use((err, req, res, next) => {
  if (err.code === "LIMIT_FILE_SIZE") {
    return res
      .status(400)
      .json({ success: false, error: "File troppo grande. Max 10MB." });
  }
  ...
  try {
    ...
    if (baseUrlEnv && !/^https?:\/\//i.test(baseUrlEnv)) {
      ...
      if (process.env.EMAIL_DEBUG)
        console.warn(
          "BASE_URL missing protocol; prefixed with https:// ->",
          baseUrlEnv
        );
    }
    ...
    if (baseUrlEnv) {
      try {
        ...
        console.warn(
          "[forgot-password] BASE_URL DNS lookup failed for",
          baseUrlEnv,
          dnsErr && dnsErr.message ? dnsErr.message : dnsErr
        );
        // Fallback: usa host corrente della richiesta (es. localhost:3000)
        baseUrl = `${req.protocol}://${req.get("host")}`;
        if (process.env.EMAIL_DEBUG)
          console.warn("[forgot-password] Falling back to", baseUrl);
      }
    }
