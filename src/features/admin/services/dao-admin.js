...
function insertImmagine(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const sql =
        "INSERT INTO IMMAGINI (url, tipo, entita_riferimento, entita_id, ordine, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *";
...
function deleteImmaginiByEntita(entitaRiferimento, entitaId) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const selectSql =
        "SELECT url FROM IMMAGINI WHERE entita_riferimento = $1 AND entita_id = $2";
      ...
      const deleteSql =
        "DELETE FROM IMMAGINI WHERE entita_riferimento = $1 AND entita_id = $2";
...
function countSquadreByCampionato(campionatoId) {
  return new Promise(async (resolve, reject) => {
    try {
      const sql =
        "SELECT COUNT(*) as count FROM CLASSIFICA WHERE campionato_id = $1";
...
function countNotiziePubblicate(dataInizio, dataFine) {
  return new Promise(async (resolve, reject) => {
    try {
      const sql =
        "SELECT COUNT(*) as count FROM NOTIZIE WHERE pubblicata = true AND data_pubblicazione >= $1 AND data_pubblicazione < $2";
...
function countEventiPubblicati(dataInizio, dataFine) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const sql =
        "SELECT COUNT(*) as count FROM EVENTI WHERE pubblicato = true AND data_inizio >= $1 AND data_inizio < $2";
      const result = await db.query(sql, [dataInizio, dataFine]);
      resolve(result.rows[0] ? result.rows[0].count || 0 : 0);
    } catch (err) {
