...
class ExportStatisticheController {
  ...
  static async esportaExcel(req, res) {
    try {
      ...
      const statistiche = await ExportStatisticheController._getStatistiche(
        req
      );
      ...
      // Crea i vari fogli
      await ExportStatisticheController._createDashboardSheet(
        workbook,
        statistiche
      );
      await ExportStatisticheController._createDettagliSheet(
        workbook,
        statistiche
      );
      await ExportStatisticheController._createPrenotazioniSheet(
        workbook,
        statistiche
      );
      await ExportStatisticheController._createUtentiSheet(
        workbook,
        statistiche
      );
      // Aggiunge un foglio con l'elenco dettagliato di tutti gli utenti
      await ExportStatisticheController._createUtentiDettagliatiSheet(
        workbook,
        statistiche
      );
      await ExportStatisticheController._createContenutoSheet(
        workbook,
        statistiche
      );
      await ExportStatisticheController._createSicurezzaSheet(
        workbook,
        statistiche
      );
      await ExportStatisticheController._createGraficiSheet(
        workbook,
        statistiche
      );
      await ExportStatisticheController._createTendenzeSheet(
        workbook,
        statistiche
      );
      await ExportStatisticheController._createCampiSheet(
        workbook,
        statistiche
      );
      await ExportStatisticheController._createRawDataSheet(
        workbook,
        statistiche
      );
  ...
  static async _createDashboardSheet(workbook, stats) {
    ...
    const kpiData = [
      {
        ...
        valore: stats.utentiTotali || 0,
        icona: "👥",
        colore: "FF3B82F6",
      },
      {
        metrica: "📰 Notizie Pubblicate",
        valore: stats.notiziePubblicate || 0,
        icona: "📰",
        colore: "FF10B981",
      },
      {
        metrica: "📅 Eventi Attivi",
        valore: stats.eventiAttivi || 0,
        icona: "📅",
        colore: "FFF59E0B",
      },
      {
        metrica: "✅ Prenotazioni Attive",
        valore: stats.prenotazioniAttive || 0,
        icona: "✅",
        colore: "FF06B6D4",
      },
      {
        metrica: "⚡ Utenti Attivi (30gg)",
        valore: stats.utentiAttivi30gg || 0,
        icona: "⚡",
        colore: "FF8B5CF6",
      },
      {
        metrica: "📊 Media Giornaliera",
        valore: stats.mediaPrenotazioniGiornaliere || 0,
        icona: "📊",
        colore: "FFEC4899",
      },
    ];
    ...
    kpiData.forEach((kpi, idx) => {
      ...
      statusCell.font = {
        color: { argb: kpi.valore > 0 ? "FF10B981" : "FFF59E0B" },
      };
    ...
    const sicurezzaData = [
      {
        metrica: "🚫 Utenti Bannati",
        valore: stats.utentiBannati || 0,
        colore: "FFDC2626",
      },
      {
        metrica: "⏸️ Utenti Sospesi",
        valore: stats.utentiSospesi || 0,
        colore: "FFF59E0B",
      },
      {
        metrica: "❌ Prenotazioni Annullate",
        valore: stats.prenotazioniAnnullate || 0,
        colore: "FF64748B",
      },
      {
        metrica: "📝 Prenotazioni con Note",
        valore: stats.prenotazioniConNote || 0,
        colore: "FF10B981",
      },
    ];
  ...
  static async _createDettagliSheet(workbook, stats) {
    ...
    const dettagli = [
      ...
      },
      {
        ...
        dati: [
          {
            nome: "Prenotazioni Totali",
            valore: stats.prenotazioniTotali || 0,
          },
          {
            nome: "Prenotazioni Attive",
            valore: stats.prenotazioniAttive || 0,
          },
          {
            nome: "Prenotazioni Confermate",
            valore: stats.prenotazioniConfermate || 0,
          },
          {
            nome: "Prenotazioni Annullate",
            valore: stats.prenotazioniAnnullate || 0,
          },
          ...
          {
            nome: "Tasso Annullamento (%)",
            valore: stats.tassoAnnullamento || 0,
          },
        ],
      },
      {
        ...
        dati: [
          ...
          {
            nome: "Eventi Totali (Storico)",
            valore: stats.eventiTotaliStorico || 0,
          },
          {
            nome: "Eventi in Arrivo (7gg)",
            valore: stats.eventiProssimi7gg || 0,
          },
      ...
      },
      {
        ...
        dati: [
          ...
          {
            nome: "Campo Più Popolare",
            valore: stats.campoPopolare?.nome || "N/D",
          },
          {
            nome: "Prenotazioni Campo Top",
            valore: stats.campoPopolare?.count || 0,
          },
        ],
      },
      {
      ...
      categoria.dati.forEach((dato) => {
        ...
        if (typeof dato.valore === "number") {
          const maxInCategory = Math.max(
            ...categoria.dati
              .filter((d) => typeof d.valore === "number")
              .map((d) => d.valore)
          );
          const barLength =
            maxInCategory > 0
              ? Math.round((dato.valore / maxInCategory) * 15)
              : 0;
  ...
  static async _createPrenotazioniSheet(workbook, stats) {
    ...
    const riepilogo = [
      [
        "📊 Totale Prenotazioni",
        stats.prenotazioniTotali || 0,
        "Storico completo",
      ],
      [
        "✅ Confermate",
        stats.prenotazioniConfermate || 0,
        "Prenotazioni confermate",
      ],
      ["⏳ Attive", stats.prenotazioniAttive || 0, "In corso o future"],
      [
        "✔️ Completate",
        stats.prenotazioniCompletate || 0,
        "Terminate con successo",
      ],
      ["❌ Annullate", stats.prenotazioniAnnullate || 0, "Cancellate"],
      ["📅 Oggi", stats.prenotazioniOggi || 0, "Prenotazioni odierne"],
      [
        "📈 Media Giornaliera",
        stats.mediaPrenotazioniGiornaliere || 0,
        "Ultimi 30 giorni",
      ],
      [
        "📝 Con Note",
        stats.prenotazioniConNote || 0,
        "Con informazioni aggiuntive",
      ],
      [
        "✓ Tasso Conferma",
        `${stats.tassoConferma || 0}%`,
        "Percentuale conferme",
      ],
      [
        "⚠️ Tasso Annullamento",
        `${stats.tassoAnnullamento || 0}%`,
        "Percentuale annullamenti",
      ],
    ];
    ...
    // Headers
    const headerRow = sheet.getRow(currentRow);
    ["Metrica", "Valore", "Descrizione", "Performance", "Trend"].forEach(
      (h, idx) => {
        const cell = headerRow.getCell(idx + 1);
        cell.value = h;
        cell.font = { bold: true, color: { argb: "FFFFFFFF" } };
    ...
    riepilogo.forEach((item, idx) => {
      ...
      if (item[0].includes("Annullate") || item[0].includes("Annullamento")) {
        performance =
          valore > 20 ? "🔴 Alto" : valore > 10 ? "🟡 Medio" : "🟢 Basso";
      } else {
        performance =
          valore > 50
            ? "🟢 Ottimo"
    ...
    const campoData = [
      ...
      [
        ...
        `${
          stats.campoPopolare?.count && stats.prenotazioniTotali
            ? (
                (stats.campoPopolare.count / stats.prenotazioniTotali) *
                100
              ).toFixed(1)
      ...
      },
      {
        ...
        items: [
          ...
          [
            "Tasso Engagement",
            `${
              stats.utentiTotali > 0
                ? ((stats.utentiAttivi30gg / stats.utentiTotali) * 100).toFixed(
                    1
                  )
                : 0
            }%`,
        ...
        items: [
          ...
          [
            ...
            `${
              stats.utentiTotali > 0
                ? (
                    ((stats.utentiBannati + stats.utentiSospesi) /
                      stats.utentiTotali) *
                    100
                  ).toFixed(1)
                : 0
    ...
    utentiData.forEach((sezione, idx) => {
      ...
      sezione.items.forEach((item, itemIdx) => {
        ...
        if (item[0].includes("Bannati") || item[0].includes("Sospesi")) {
          status =
            valore === 0 ? "✅ OK" : valore < 5 ? "⚠️ Attenzione" : "🔴 Alto";
        } else {
  ...
  static async _createContenutoSheet(workbook, stats) {
    ...
    const contenutoData = [
      {
        ...
        stats: [
          ...
          [
            "Media Settimanale",
            stats.notizie7gg ? Math.round((stats.notizie7gg / 7) * 10) / 10 : 0,
          ],
        ],
      },
      {
    ...
    contenutoData.forEach((sezione) => {
      ...
      sezione.stats.forEach((stat, idx) => {
        ...
        // Barra grafica
        const maxInSection = Math.max(
          ...sezione.stats.map((s) => (typeof s[1] === "number" ? s[1] : 0))
        );
        const barLength =
          maxInSection > 0 ? Math.round((stat[1] / maxInSection) * 12) : 0;
        row.getCell(3).value = "▓".repeat(barLength);
  ...
  static async _createSicurezzaSheet(workbook, stats) {
      ...
      },
      {
        ...
        items: [
          [
            ...
            stats.prenotazioniAnnullate || 0,
            `${(
              (stats.prenotazioniAnnullate / totalePrenotazioni) *
              100
            ).toFixed(2)}%`,
          ],
          ...
          [
            ...
            stats.prenotazioniConNote || 0,
            `${((stats.prenotazioniConNote / totalePrenotazioni) * 100).toFixed(
              2
            )}%`,
          ],
        ],
      },
      {
        ...
        items: [
          ...
          [
            ...
            `${
              totaleUtenti > 0
                ? ((stats.utentiAttivi30gg / totaleUtenti) * 100).toFixed(1)
                : 0
            }%`,
            "Buono > 50%",
          ],
    ...
    sicurezzaData.forEach((sezione) => {
      ...
      // Headers colonne
      const headerRow = sheet.getRow(currentRow);
      ["Indicatore", "Valore", "Percentuale", "Valutazione", "Action"].forEach(
        (h, idx) => {
          const cell = headerRow.getCell(idx + 1);
          cell.value = h;
          cell.font = { bold: true, color: { argb: "FFFFFFFF" } };
      ...
      sezione.items.forEach((item, idx) => {
        ...
        if (
          item[0].includes("Bannati") ||
          item[0].includes("Sospesi") ||
          item[0].includes("Annullate")
        ) {
          valutazione =
            valore === 0
              ? "✅ Ottimo"
  ...
  static async _createGraficiSheet(workbook, stats) {
    ...
    const utentiData = [
      ...
      [
        ...
        stats.utentiBannati || 0,
        `${(
          ((stats.utentiBannati || 0) / (stats.utentiTotali || 1)) *
          100
        ).toFixed(1)}%`,
        "",
      ],
      [
        ...
        stats.utentiSospesi || 0,
        `${(
          ((stats.utentiSospesi || 0) / (stats.utentiTotali || 1)) *
          100
        ).toFixed(1)}%`,
        "",
      ],
        ...
        if (idx === 0) {
          ...
          if (colIdx === 3 && idx > 0) {
            ...
            excelCell.font = {
              color: {
                argb:
                  idx === 1 ? "FF10B981" : idx === 2 ? "FFDC2626" : "FFF59E0B",
              },
            };
          }
        }
    ...
    const prenotazioniData = [
      ...
      [
        "⏳ Attive",
        stats.prenotazioniAttive || 0,
        `${(
          ((stats.prenotazioniAttive || 0) / (stats.prenotazioniTotali || 1)) *
          100
        ).toFixed(1)}%`,
        "",
      ],
    ...
    const temporaleData = [
      ...
      [
        ...
        "-",
        stats.notizie7gg || 0,
        stats.eventiProssimi7gg || 0,
      ],
      [
        "📊 Ultimi 30gg",
        stats.mediaPrenotazioniGiornaliere
          ? Math.round((stats.mediaPrenotazioniGiornaliere || 0) * 30)
          : 0,
    ...
    const performanceData = [
      ...
      [
        "🏟️ Campo Più Popolare",
        stats.campoPopolare?.nome || "N/D",
        `⭐ ${stats.campoPopolare?.count || 0} prenotazioni`,
      ],
      [
        "⭐ Media Recensioni",
        `${stats.mediaRecensioni || 0}/5`,
        stats.mediaRecensioni >= 4
          ? "🌟 Eccellente"
          : stats.mediaRecensioni >= 3
          ? "👍 Buono"
          : "⚠️ Da migliorare",
      ],
      [
        "💯 Tasso Conferma",
        `${stats.tassoConferma || 0}%`,
        stats.tassoConferma >= 80
          ? "✅ Ottimo"
          : stats.tassoConferma >= 60
          ? "👍 Buono"
          : "⚠️ Da migliorare",
      ],
      [
        ...
        `${
          stats.utentiTotali > 0
            ? ((stats.utentiAttivi30gg / stats.utentiTotali) * 100).toFixed(1)
            : 0
        }%`,
        (stats.utentiAttivi30gg / (stats.utentiTotali || 1)) * 100 >= 50
          ? "✅ Alto"
          : "⚠️ Medio",
      ],
    ];
  ...
  static async _createTendenzeSheet(workbook, stats) {
    ...
    if (stats.tendenzeMensili && stats.tendenzeMensili.length > 0) {
      ...
      stats.tendenzeMensili.slice(-12).forEach((mese, idx) => {
        ...
        const meseName = meseData.toLocaleDateString("it-IT", {
          month: "long",
          year: "numeric",
        });
        ...
        if (idx > 0) {
          ...
          const diffPren =
            (mese.prenotazioni || 0) - (prevMese.prenotazioni || 0);
          const diffUser =
            (mese.nuovi_utenti || 0) - (prevMese.nuovi_utenti || 0);
          trendPren =
            diffPren > 0
              ? `📈 +${diffPren}`
              : diffPren < 0
        ...
        const note =
          (mese.prenotazioni || 0) > 50
            ? "⭐ Ottimo mese"
            : (mese.prenotazioni || 0) > 30
            ? "✓ Buono"
            : "○ Normale";
  ...
  static async _createCampiSheet(workbook, stats) {
    ...
    const campiRiepilogo = [
      ...
      [
        ...
        stats.campiAttivi || 0,
        "Prenotabili",
        stats.campiAttivi === stats.campiTotali
          ? "✅ Tutti attivi"
          : "⚠️ Alcuni inattivi",
      ],
      [
      ...
      [
        "📊 Media per Campo",
        stats.campiTotali > 0
          ? Math.round((stats.prenotazioniTotali || 0) / stats.campiTotali)
          : 0,
        "Prenotazioni medie",
        "",
      ],
      ...
      [
        ...
        `${
          stats.prenotazioniTotali && stats.campiTotali
            ? (
                (stats.prenotazioniTotali / (stats.campiTotali * 365)) *
                100
              ).toFixed(1)
            : 0
        }%`,
  ...
  static async _createRawDataSheet(workbook, stats) {
    ...
    sheet.getCell("A2").value =
      "Questi sono tutti i dati grezzi in formato chiave-valore per ulteriori elaborazioni";
  ...
  static async _createUtentiDettagliatiSheet(workbook, stats) {
    ...
    for (const u of users) {
      try {
        ...
        const totalePren = Array.isArray(prenotazioni)
          ? prenotazioni.length
          : 0;
        ...
        console.error(
          "[Export] Errore recupero dettagli utente id=",
          u.id,
          err
        );
      }
